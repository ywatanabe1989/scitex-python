...
async def click_nature_go():
    ...
    try:
        ...
        # Look for the GO button next to "Available from Nature"
        # Based on the screenshot, it should be in the "Online Sources" section
        go_buttons = await page.evaluate('''() => {
            // Find all GO buttons (input elements with value="GO")
            ...
            return goButtons.map((btn, index) => {
                ...
                return {
                    ...
                    value: btn.value,
                    rowText: rowText,
                    isNature: rowText.toLowerCase().includes('nature')
                };
            });
        }''')
        ...
        for i, btn in enumerate(go_buttons):
            print(f'  [{i}] {btn["value"]} - Nature: {btn["isNature"]}')
            print(f'      Context: {btn["rowText"][:100]}...')
            
            if btn["isNature"]:
                nature_button_index = i
                print(f'      🎯 NATURE GO BUTTON FOUND!')
        
        if nature_button_index >= 0:
            print(f'\\nStep 3: Clicking Nature GO button (index {nature_button_index})...')
            
            # Handle popup and click the Nature GO button
            popup_promise = page.wait_for_event('popup', timeout=30000)
            
            await page.evaluate(f'''() => {{
                const goButtons = Array.from(document.querySelectorAll('input[value="GO"]'));
                const natureButton = goButtons[{nature_button_index}];
                if (natureButton) {{
            ...
            # Take screenshot
            await popup.screenshot(path='nature_institutional_final.png', full_page=True)
            print('📸 Screenshot: nature_institutional_final.png')
            ...
            if 'nature.com' in popup_url.lower():
                ...
                if has_access and not has_paywall:
                    ...
                    print('⚠️  Partial access detected')
            ...
            print('\\n' + '='*70)
            print('🎉 NATURE NEUROSCIENCE INSTITUTIONAL ACCESS TEST COMPLETE!')
            print('='*70)
            print('✅ OpenAthens authentication: WORKING') 
            print('✅ University of Melbourne OpenURL resolver: WORKING')
            print('✅ Nature institutional access: TESTED')
            ...
            print('Available GO button contexts:')
            for i, btn in enumerate(go_buttons):
                print(f'  [{i}] {btn["rowText"][:100]}...')
