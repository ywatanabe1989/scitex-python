...
class TestListMembers:
    ...
    def test_members_class_target(self):
        """Test listing members of a class."""
        from scitex.introspect import list_members

        result = list_members("pathlib.Path")
        assert result["success"] is True
        assert result["count"] > 0

    def test_members_invalid_path(self):
        """Test invalid path returns error."""
        from scitex.introspect import list_members

        result = list_members("nonexistent.module")
