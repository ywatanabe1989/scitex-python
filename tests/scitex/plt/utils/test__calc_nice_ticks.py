...
class TestCalcNiceTicks:
    ...
    def test_calc_nice_ticks_prefer_integer_true(self):
        ...
        assert any(isinstance(tick, int) for tick in ticks)
    ...
    def test_calc_nice_ticks_matplotlib_integration(self):
        ...
        with patch(
            "figrecipe._utils._calc_nice_ticks.mticker.MaxNLocator"
    ...
    def test_calc_nice_ticks_too_many_ticks_handling(self):
        ...
        with patch(
            ...
            mock_locator1.tick_values.return_value = np.array(
                [0, 1, 2, 3, 4, 5, 6]
            )  # 7 ticks
    ...
    def test_calc_nice_ticks_edge_filtering(self):
        ...
        with patch(
            ...
            mock_locator = MagicMock()
            mock_locator_class.return_value = mock_locator
            mock_locator.tick_values.return_value = np.array(
                [-1, 0, 2, 4, 6, 8, 10, 11]
            )

            min_val, max_val = 0, 10
            ticks = calc_nice_ticks(
                min_val, max_val, allow_edge_min=False, allow_edge_max=False
            )
    ...
    def test_calc_nice_ticks_real_world_data_scenarios(self):
        ...
        assert 0 in percentage_ticks or any(
            abs(tick) < 1e-10 for tick in percentage_ticks
        )
    ...
    def test_calc_nice_ticks_return_type_consistency(self):
        ...
        test_cases = [(0, 10), (-5, 5), (1e-6, 1e-3), (1000, 10000), (0.1, 0.9)]
