...
# ----------------------------------------
"""Comprehensive tests for collect_figure_metadata function.

Tests the plot type detection system, ensuring matplotlib's internal method calls
(e.g., boxplot internally calling plot()) don't incorrectly identify plot types.
"""
...
class TestPlotTypeDetection:
    """Test that plot types are correctly detected via history tracking."""

    def test_boxplot_detected_as_boxplot(self):
        """Test boxplot is correctly identified, not as 'line'."""
    ...
    def test_line_plot_detected_as_line(self):
        """Test basic line plot detection."""
        fig, ax = splt.subplots()
        x = np.linspace(0, 10, 100)
        ax.plot(x, np.sin(x))

        metadata = collect_figure_metadata(fig._fig_mpl, ax)
        assert metadata.get("plot_type") == "line"
        assert metadata.get("method") == "plot"

        splt.close(fig)

    def test_scatter_detected_as_scatter(self):
        ...
        metadata = collect_figure_metadata(fig._fig_mpl, ax)
        assert metadata.get("plot_type") == "scatter"
        assert metadata.get("method") == "scatter"

        splt.close(fig)

    def test_bar_detected_as_bar(self):
        """Test bar plot detection."""
        fig, ax = splt.subplots()
        ax.bar([1, 2, 3], [4, 5, 6])

        metadata = collect_figure_metadata(fig._fig_mpl, ax)
        assert metadata.get("plot_type") == "bar"
        assert metadata.get("method") == "bar"

        splt.close(fig)

    def test_hist_detected_as_hist(self):
        ...
        metadata = collect_figure_metadata(fig._fig_mpl, ax)
        assert metadata.get("plot_type") == "hist"
        assert metadata.get("method") == "hist"

        splt.close(fig)

    def test_imshow_detected_as_image(self):
        """Test image/heatmap detection via imshow."""
        fig, ax = splt.subplots()
        data = np.random.randn(10, 10)
        ax.imshow(data)

        metadata = collect_figure_metadata(fig._fig_mpl, ax)
        assert metadata.get("plot_type") == "image"
        assert metadata.get("method") == "imshow"

        splt.close(fig)

    def test_violinplot_detected_as_violin(self):
        """Test violin plot detection."""
        fig, ax = splt.subplots()
        data = [np.random.randn(30) for _ in range(3)]
        ax.violinplot(data)

        metadata = collect_figure_metadata(fig._fig_mpl, ax)
        assert metadata.get("plot_type") == "violin"
        assert metadata.get("method") == "violinplot"

        splt.close(fig)

    def test_errorbar_detected_as_errorbar(self):
        """Test errorbar plot detection."""
        fig, ax = splt.subplots()
        x = [1, 2, 3, 4]
        y = [2, 4, 6, 8]
        yerr = [0.5, 0.3, 0.4, 0.2]
        ax.errorbar(x, y, yerr=yerr)

        metadata = collect_figure_metadata(fig._fig_mpl, ax)
        assert metadata.get("plot_type") == "errorbar"
        assert metadata.get("method") == "errorbar"

        splt.close(fig)

    def test_fill_between_detected_as_fill(self):
        """Test fill_between detection."""
        fig, ax = splt.subplots()
        x = np.linspace(0, 10, 100)
        y1 = np.sin(x)
        y2 = np.sin(x) + 0.5
        ...
        metadata = collect_figure_metadata(fig._fig_mpl, ax)
        assert metadata.get("plot_type") == "fill"
        assert metadata.get("method") == "fill_between"

        splt.close(fig)


class TestScitexWrapperAccess:
    """Test that scitex wrapper is correctly accessed for history tracking."""

    def test_scitex_wrapper_stored_on_mpl_axes(self):
        """Test that _scitex_wrapper is stored on matplotlib axes."""
        fig, ax = splt.subplots()
        ax.plot([1, 2, 3], [1, 2, 3])
    ...
    def test_scitex_wrapper_has_history(self):
        """Test that scitex wrapper has history attribute."""
        fig, ax = splt.subplots()
        ax.boxplot([np.random.randn(30)])

        mpl_ax = ax._axis_mpl
        wrapper = mpl_ax._scitex_wrapper

        assert hasattr(wrapper, "history"), "scitex wrapper should have history"
        assert len(wrapper.history) > 0, "history should contain boxplot call"

        splt.close(fig)

    def test_metadata_via_raw_mpl_axes_finds_wrapper(self):
        """Test that collect_figure_metadata finds wrapper from raw mpl axes."""
        fig, ax = splt.subplots()
        ax.boxplot([np.random.randn(30)])

        # Pass raw matplotlib axes (simulates scitex.io.save path)
        mpl_ax = ax._axis_mpl
        metadata = collect_figure_metadata(fig._fig_mpl, mpl_ax)

        assert metadata.get("plot_type") == "boxplot"
        assert metadata.get("method") == "boxplot"

        splt.close(fig)


class TestHistoryFormat:
    """Test that history is correctly formatted and parsed."""

    def test_history_tuple_format(self):
        """Test that history entries are tuples with (id, method, tracked_dict, kwargs)."""
        fig, ax = splt.subplots()
        ax.boxplot([np.random.randn(30)], labels=["Test"])
    ...
    def test_boxplot_history_method_name(self):
        """Test that boxplot is recorded with correct method name in history."""
        fig, ax = splt.subplots()
        ax.boxplot([np.random.randn(30)])
...
class TestMultipleAxes:
    """Test plot type detection with multiple axes."""

    def test_subplot_each_axis_has_wrapper(self):
        """Test that each axis in subplots has its own _scitex_wrapper."""
        fig, axes = splt.subplots(nrows=2, ncols=2)

        # Access axes via flatten() method - returns flat list of AxisWrapper
    ...
    def test_different_plot_types_per_axis(self):
        """Test different plot types in different axes are correctly detected."""
        # Test with single axis since subplots may return AxisWrapper
...
class TestEdgeCases:
    """Test edge cases in plot type detection."""

    def test_empty_axes(self):
        """Test detection with empty axes."""
        fig, ax = splt.subplots()
        # No plots added

        metadata = collect_figure_metadata(fig._fig_mpl, ax)
        # Should not crash, plot_type might be None
        assert "matplotlib" in metadata  # Basic metadata should still exist

        splt.close(fig)

    def test_multiple_plot_calls_same_axis(self):
        """Test detection when multiple plot calls on same axis."""
        fig, ax = splt.subplots()
        ax.plot([1, 2, 3], [1, 2, 3])
        ax.plot([1, 2, 3], [3, 2, 1])
    ...
    def test_mixed_plot_and_scatter(self):
        """Test when both plot and scatter are called (first wins)."""
        fig, ax = splt.subplots()
        ax.plot([1, 2, 3], [1, 2, 3])
        ...
        assert metadata.get("plot_type") in ["line", "scatter"]

        splt.close(fig)


class TestInternalMethodCallsNotMisidentified:
    """Test that matplotlib's internal method calls don't cause misidentification.

    This is the core issue: matplotlib functions like boxplot() internally call
    plot() multiple times, which could cause boxplot to be misidentified as line.
    """

    def test_boxplot_not_line_even_with_many_lines(self):
        """Ensure boxplot is not misidentified as line despite internal plot() calls."""
        fig, ax = splt.subplots()

        # Boxplot creates multiple internal lines (whiskers, caps, etc.)
        ax.boxplot([np.random.randn(30) for _ in range(5)])

        # Check we have lines (from internal plot calls)
        assert len(ax._axis_mpl.lines) > 0, "Boxplot should create lines internally"

        # But should still be detected as boxplot
        metadata = collect_figure_metadata(fig._fig_mpl, ax)
        assert metadata.get("plot_type") == "boxplot"
        assert metadata.get("method") == "boxplot"
    ...
    def test_boxplot_no_traces_exported(self):
        """Ensure boxplot's internal lines are not exported as traces."""
        fig, ax = splt.subplots()
        ax.boxplot([np.random.randn(30) for _ in range(3)])

        # Internal lines exist
        assert len(ax._axis_mpl.lines) > 0, "Boxplot creates internal lines"
    ...
    def test_errorbar_not_line_despite_internal_calls(self):
        """Ensure errorbar is not misidentified as line."""
        fig, ax = splt.subplots()
        ax.errorbar([1, 2, 3], [1, 2, 3], yerr=[0.1, 0.2, 0.1])
    ...
    def test_hist_not_misidentified(self):
        """Ensure histogram patches don't get misidentified."""
        fig, ax = splt.subplots()
...
class TestCsvJsonConsistency:
    ...
    def test_verify_consistency_simple_plot(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            ax.plot(x, np.sin(x), id="test_sin")
            ...
            assert len(result["extra_in_csv"]) == 0
    ...
    def test_verify_consistency_boxplot(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            fig, ax = splt.subplots()
            np.random.seed(42)
            data = [np.random.randn(30), np.random.randn(30) + 1]
            ax.boxplot(data, id="test_box")

            save_path = os.path.join(tmpdir, "test_boxplot.png")
            scitex.io.save(fig, save_path)
            splt.close(fig)
    ...
    def test_verify_consistency_multiple_traces(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            save_path = os.path.join(tmpdir, "test_multi.png")
            scitex.io.save(fig, save_path)
            splt.close(fig)
            ...
            assert result["valid"] is True
            assert len(result["csv_columns"]) >= 4  # x, y for each trace
    ...
    def test_verify_consistency_scatter(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            x = np.random.randn(50)
            y = np.random.randn(50)
            ax.scatter(x, y, id="test_scatter")

            save_path = os.path.join(tmpdir, "test_scatter.png")
            scitex.io.save(fig, save_path)
            splt.close(fig)
    ...
    def test_verify_consistency_hist(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            ax.hist(data, bins=10, id="test_hist")

            save_path = os.path.join(tmpdir, "test_hist.png")
            scitex.io.save(fig, save_path)
            splt.close(fig)
...
class TestAssertCsvJsonConsistency:
    """Tests for assert_csv_json_consistency function."""
...
class TestJsonMetadataFields:
    """Tests for JSON metadata field presence."""
    ...
    def test_json_has_csv_columns_actual(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert "csv_columns_actual" in metadata
            assert isinstance(metadata["csv_columns_actual"], list)
            assert len(metadata["csv_columns_actual"]) > 0
    ...
    def test_json_has_csv_hash(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert metadata["csv_hash"] is not None
            assert isinstance(metadata["csv_hash"], str)
            assert len(metadata["csv_hash"]) == 16
    ...
    def test_json_csv_columns_actual_matches_csv(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(json_path) as f:
                metadata = json.load(f)
            json_cols = metadata["csv_columns_actual"]

            assert csv_cols == json_cols


class TestCsvHashComputation:
    """Tests for CSV hash computation functions."""
    ...
    def test_compute_hash_from_df_returns_string(self):
        ...
        result = _compute_csv_hash_from_df(df)
    ...
    def test_compute_hash_deterministic(self):
        ...
        hash1 = _compute_csv_hash_from_df(df)
        hash2 = _compute_csv_hash_from_df(df)
        assert hash1 == hash2
    ...
    def test_compute_hash_different_data_different_hash(self):
        ...
        df2 = pd.DataFrame({"x": [1.0, 2.0, 4.0]})
...
# --------------------------------------------------------------------------------
# Start of Source Code from: /home/ywatanabe/proj/scitex-code/src/scitex/plt/utils/_collect_figure_metadata.py
# --------------------------------------------------------------------------------
# #!/usr/bin/env python3
# # -*- coding: utf-8 -*-
...
# --------------------------------------------------------------------------------
# End of Source Code from: /home/ywatanabe/proj/scitex-code/src/scitex/plt/utils/_collect_figure_metadata.py
# --------------------------------------------------------------------------------
