...
def test_add_hue_col_existing_hue_column():
    ...
    df = pd.DataFrame({"col1": [1, 2, 3], "hue": [10, 20, 30]})  # Existing hue column
...
def test_add_hue_col_categorical_data():
    ...
    df = pd.DataFrame({"cat_col": pd.Categorical(["A", "B", "A", "C"])})
...
def test_add_hue_col_datetime_data():
    ...
    df = pd.DataFrame({"date_col": pd.date_range("2023-01-01", periods=3)})
...
def test_add_hue_col_mixed_types():
    ...
    df = pd.DataFrame(
        {"num": [1, 2, 3], "text": ["x", "y", "z"], "mixed": [1, "two", 3.0]}
    )
