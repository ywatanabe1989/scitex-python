...
class TestSubplotsWrapper:
    ...
    def test_single_axis(self):
        ...
        assert hasattr(ax, "plot"), "Should have plot method"
    ...
    def test_backward_compatibility_flat_iteration(self):
        """Test that flat iteration works via .flat for backward compatibility."""
        fig, axes = scitex.plt.subplots(4, 3)

        # Test flat iteration yields all 12 axes
        ax_list = list(axes.flat)
        assert len(ax_list) == 12, "Flat iteration should yield 12 axes"
    ...
    def test_multi_axes_plotting(self):
        """Test plotting on multiple axes."""
        ...
        # Verify each axis has plotted data
        for i in range(2):
            for j in range(2):
                assert len(axes[i, j].lines) > 0, f"Axis [{i},{j}] should have lines"
    ...
    def test_matplotlib_compatibility(self):
        ...
        assert (
            scitex_axes.shape == mpl_axes.shape
        ), "Should have same shape as matplotlib"
