...
from scitex.plt._subplots._export_as_csv_formatters._format_imshow import (  # noqa: E402
    _format_imshow,
)
...
class TestFormatImshow:
    ...
    def test_args_2d_grayscale_image(self):
        ...
        assert "r0c0_plot-0_row" in result.columns
        assert "r0c0_plot-0_col" in result.columns
        assert "r0c0_plot-0_value" in result.columns
        assert len(result) == 4  # 2x2 = 4 pixels

        # Check values are correct
        expected_values = [0.1, 0.2, 0.3, 0.4]
        np.testing.assert_array_almost_equal(
            result["r0c0_plot-0_value"].values, expected_values
        )
    ...
    def test_args_3d_rgb_image(self):
        ...
        assert "r0c0_plot-0_row" in result.columns
        assert "r0c0_plot-0_col" in result.columns
        assert "r0c0_plot-0_r" in result.columns
        assert "r0c0_plot-0_g" in result.columns
        assert "r0c0_plot-0_b" in result.columns
        assert len(result) == 4  # 2x2 = 4 pixels
    ...
    def test_args_3d_rgba_image(self):
        ...
        assert "r0c0_plot-0_g" in result.columns
        assert "r0c0_plot-0_b" in result.columns
        assert "r0c0_plot-0_a" in result.columns
    ...
    def test_row_col_indices_correct(self):
        ...
        np.testing.assert_array_equal(result["r0c0_plot-0_row"].values, expected_rows)
        np.testing.assert_array_equal(result["r0c0_plot-0_col"].values, expected_cols)
