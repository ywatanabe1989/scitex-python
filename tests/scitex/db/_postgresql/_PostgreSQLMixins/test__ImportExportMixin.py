...
"""
    ...
    def test_save_to_csv_specific_columns(self, mixin):
        ...
        expected_sql = (
            "COPY (SELECT id, name, email FROM users) TO STDOUT WITH CSV HEADER"
        )
        actual_sql = mixin.cursor.copy_expert.call_args[1]["sql"]
        assert actual_sql == expected_sql
    ...
    def test_save_to_csv_with_where_clause(self, mixin):
        ...
        expected_sql = (
            "COPY (SELECT * FROM users WHERE active = true) TO STDOUT WITH CSV HEADER"
        )
        actual_sql = mixin.cursor.copy_expert.call_args[1]["sql"]
        assert actual_sql == expected_sql
    ...
    def test_save_to_csv_with_columns_and_where(self, mixin):
        ...
        expected_sql = (
            "COPY (SELECT id, name FROM users WHERE age > 25) TO STDOUT WITH CSV HEADER"
        )
        actual_sql = mixin.cursor.copy_expert.call_args[1]["sql"]
        assert actual_sql == expected_sql
