...
class TestPostgreSQLBatchMixin:
    ...
    def test_insert_many_simple(self, mixin):
        ...
        mixin.executemany.assert_called_once_with(
            expected_query, [(1, "John", 25), (2, "Jane", 30), (3, "Bob", 35)]
        )
    ...
    def test_insert_many_with_batch_size(self, mixin):
        ...
        records = [{"id": i, "value": f"test{i}"} for i in range(10)]
    ...
    def test_prepare_batch_parameters(self, mixin):
        ...
        expected = [("Alice", 90, True), ("Bob", 85, False), ("Charlie", 95, True)]
    ...
    def test_dataframe_to_sql_fail_mode(self, mixin):
        ...
        assert not any(
            "DROP TABLE" in str(call) for call in mixin.execute.call_args_list
        )
        assert not any(
            "CREATE TABLE" in str(call) for call in mixin.execute.call_args_list
        )
    ...
    def test_large_batch_processing(self, mixin):
        ...
        large_records = [
            {"id": i, "data": f"data_{i}", "value": i * 10} for i in range(1000)
        ]
    ...
    def test_records_with_null_values(self, mixin):
        ...
        expected_params = [(1, "Test", None), (2, None, "Value"), (3, "Another", None)]
    ...
    def test_consistent_column_order(self, mixin):
        ...
        expected_params = [(2, 1, 3), (5, 4, 6), (8, 7, 9)]  # b, a, c order
