...
class TestPostgreSQLBlobMixin:
    ...
    def test_save_array_single(self, mixin, sample_arrays):
        ...
        with patch("psycopg2.Binary") as mock_binary:
            ...
            # Verify INSERT query
            expected_query = (
                "INSERT INTO test_table (data,data_dtype,data_shape) VALUES (%s,%s,%s)"
            )
            mixin.execute.assert_called_once_with(
                expected_query, [b"mock_binary_data", "int32", "(3, 2)"]
            )
    ...
    def test_save_array_with_id(self, mixin, sample_arrays):
        ...
        with patch("psycopg2.Binary") as mock_binary:
            ...
            # Verify UPDATE query
            expected_query = (
                "UPDATE test_table SET data=%s,data_dtype=%s,data_shape=%s WHERE id=%s"
            )
            mixin.execute.assert_called_once_with(
                expected_query, [b"mock_binary", "float32", "(5,)", 42]
            )
    ...
    def test_load_array_single_id(self, mixin, sample_arrays):
        ...
        mixin.cursor.fetchall.return_value = [(42, blob_data, "int32", "(3, 2)")]
    ...
    def test_load_array_without_metadata(self, mixin):
        ...
        result = mixin.load_array(
            "test_table", "data", ids=1, dtype=np.float64, shape=(3,)
        )
    ...
    def test_binary_to_array_without_metadata(self, mixin):
        ...
        result = mixin.binary_to_array(binary, dtype=np.float32, shape=(3,))
    ...
    def test_get_array_dict(self, mixin):
        ...
        mixin.binary_to_array = MagicMock(
            side_effect=[
                np.array([1, 2]),
                np.array([3, 4]),  # array1
                np.array([5, 6, 7]),
                np.array([8, 9, 10]),  # array2
            ]
        )
