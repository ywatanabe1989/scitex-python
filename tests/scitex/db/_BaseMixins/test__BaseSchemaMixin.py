...
class TestBaseSchemaMixin:
    ...
    def test_create_index_with_name_not_implemented(self):
        ...
        with pytest.raises(NotImplementedError):
            self.mixin.create_index(
                "users", ["email", "username"], "idx_users_email_username"
            )
    ...
    def test_method_signatures(self):
        ...
        assert sig.return_annotation == List[str] or str(
        ...
        assert sig.return_annotation == List[Dict[str, Any]] or str(
            sig.return_annotation
        ).startswith("typing.List")
        ...
        class DatabaseWithSchema(_BaseSchemaMixin):
            ...
            def create_index(self, table, columns, index_name=None):
                ...
                self.schema[table]["indexes"].append(
                    {"name": index_name, "columns": columns, "unique": False}
                )
    ...
    def test_documentation(self):
        ...
        assert _BaseSchemaMixin.__doc__ is None or isinstance(
            _BaseSchemaMixin.__doc__, str
        )
