...
pytestmark = pytest.mark.skip(
    reason="Dependencies not available - test_pip_install_latest functions are commented out"
)
...
class TestInstallPackage:
    ...
    def test_install_package_special_characters(self, mock_call):
        ...
        assert result == 0
        expected_cmd = (
            "pip install git+https://github.com/user-name/repo-name@v1.0.0-beta"
        )
        mock_call.assert_called_once_with(expected_cmd, shell=True)
...
class TestMainFunction:
    ...
    def test_main_success(self, mock_log, mock_install, mock_get_tag):
        ...
        assert any(
            "Installing user/repo at tag v1.2.3" in str(call) for call in log_calls
        )
    ...
    def test_main_installation_failure(
        self, mock_log_error, mock_install, mock_get_tag
    ):
