...
def create_figure_01_multi_type(output_dir: Path) -> Path:
    ...
    axes[0, 0].plot(
        x, y1, "o", markersize=2, alpha=0.5, label="Data points", id="data-points"
    )
    ...
    axes[0, 1].errorbar(
        x[::10], y_mean[::10], yerr=y_err[::10],
        fmt="o", markersize=4, capsize=2, label="Error bars", id="errorbars"
    )
    ...
    for i, (data, label) in enumerate(zip(group_data, groups)):
        ...
        axes[1, 0].scatter(
            np.full_like(data, i) + jitter, data,
            alpha=0.3, s=10, id=f"scatter-{label}"
        )
    ...
    axes[1, 1].hist(
        hist_data, bins=25, density=True, alpha=0.6, label="Histogram", id="hist"
    )
    ...
    for i, (data, label) in enumerate(zip(group_data, groups)):
        axes[1, 1].axvline(
            np.mean(data), linestyle="--", alpha=0.7,
            label=f"{label} mean", id=f"mean-{label}"
        )
...
def create_figure_02_scientific(output_dir: Path) -> Path:
    ...
    axes[0, 0].stx_fillv([1, 3], [2, 4], alpha=0.2, color="red", id="stim-period")
    axes[0, 0].stx_rectangle(
        1, ch1.min() - 0.5, 1, ch3.max() - ch1.min() + 1,
        edgecolor="red", facecolor="none", linestyle="--", id="stim-box"
    )
    ...
    axes[0, 1].stx_shaded_line(
        trace_x, mean_trace - std_trace, mean_trace, mean_trace + std_trace,
        id="mean-std"
    )
    ...
    for i, (data, label) in enumerate(zip(group_data, groups)):
        ...
        axes[1, 0].scatter(
            np.full_like(data, i) + jitter, data,
            alpha=0.4, s=8, id=f"strip-{label}"
        )
    axes[1, 0].stx_violin(group_data, labels=groups, id="violin")
    for i, data in enumerate(group_data):
        axes[1, 0].plot(
            i, np.mean(data), "D", color="red", markersize=6, id=f"mean-marker-{i}"
        )
    ...
    axes[1, 1].plot(
        reg_x, reg_y, "-", color="red", linewidth=2,
        label=f"y={slope:.2f}x+{intercept:.2f}", id="regression"
    )
    ci = 0.3
    ...
    axes[1, 2].plot(
        [0, 0, 2, 2], [y_max, y_max + 0.1, y_max + 0.1, y_max],
        "k-", linewidth=1, id="sig-bracket"
    )
    axes[1, 2].text(1, y_max + 0.15, "***", ha="center", va="bottom", fontsize=10)
...
def create_figure_03_neuroscience(output_dir: Path) -> Path:
    ...
    for trial in range(n_trials):
        ...
        spike_trains.append(np.sort(np.concatenate([pre_stim, stim, post_stim])))
    ...
    axes[0, 0].axvline(0.5, color="green", linestyle="--", linewidth=1, id="stim-onset")
    axes[0, 0].axvline(1.5, color="red", linestyle="--", linewidth=1, id="stim-offset")
    ...
    axes[0, 1].bar(
        bin_centers, firing_rate, width=bin_edges[1] - bin_edges[0],
        alpha=0.6, id="psth"
    )
    smooth_rate = gaussian_filter1d(firing_rate.astype(float), sigma=2)
    axes[0, 1].plot(
        bin_centers, smooth_rate, "-", color="red", linewidth=2,
        label="Smoothed", id="smooth-rate"
    )
    axes[0, 1].stx_fillv([0.5], [1.5], alpha=0.2, color="yellow", id="stim-period")
    ...
    for i in range(n_waveforms):
        waveform = -np.exp(-((waveform_time - 0.3) ** 2) / 0.02) + \
                   0.3 * np.exp(-((waveform_time - 0.8) ** 2) / 0.05)
        waveform += 0.05 * np.random.randn(len(waveform_time))
        axes[1, 0].plot(
            waveform_time, waveform, alpha=0.2, color="blue", id=f"waveform-{i}"
        )
    mean_waveform = -np.exp(-((waveform_time - 0.3) ** 2) / 0.02) + \
                    0.3 * np.exp(-((waveform_time - 0.8) ** 2) / 0.05)
    axes[1, 0].plot(
        waveform_time, mean_waveform, "-", color="red", linewidth=2,
        label="Mean", id="mean-waveform"
    )
    ...
    axes[1, 1].plot(
        isi_x, exp_fit, "-", color="red", linewidth=2,
        label=f"Exp fit (tau={mean_isi:.1f}ms)", id="exp-fit"
    )
    ...
    responses = 10 + 8 * np.exp(-((orientations - 90) ** 2) / (2 * 30**2)) + \
                np.random.randn(len(orientations))
    response_sem = 1 + 0.5 * np.random.rand(len(orientations))

    axes[2, 0].errorbar(
        orientations, responses, yerr=response_sem,
        fmt="o", capsize=3, id="tuning-data"
    )
    fit_x = np.linspace(0, 180, 100)
    fit_y = 10 + 8 * np.exp(-((fit_x - 90) ** 2) / (2 * 30**2))
    axes[2, 0].plot(
        fit_x, fit_y, "-", color="red", linewidth=2,
        label="Gaussian fit", id="tuning-fit"
    )
    axes[2, 0].axvline(90, color="gray", linestyle="--", alpha=0.5, id="preferred")
    ...
    for i in range(n_neurons):
        ...
        pop_activity[i] = np.exp(-((np.arange(n_time) - peak_time) ** 2) / (2 * 10**2))
        pop_activity[i] += 0.1 * np.random.randn(n_time)

    axes[2, 1].imshow(pop_activity, aspect="auto", cmap="hot", id="pop-heatmap")
...
def main(
    ...
    figure : int
        Figure to edit: 1 (multi-type), 2 (scientific), 3 (neuroscience)
    """
    ...
    # Create all figures
    creators = {
        1: ("Multi-type per axis", create_figure_01_multi_type),
        2: ("Scientific figure", create_figure_02_scientific),
        3: ("Neuroscience figure", create_figure_03_neuroscience),
    }

    if figure not in creators:
        logger.error(f"Invalid figure number: {figure}. Choose 1, 2, or 3.")
        return 1

    name, creator = creators[figure]
    logger.info(f"Creating figure {figure}: {name}")
    json_path = creator(out)
