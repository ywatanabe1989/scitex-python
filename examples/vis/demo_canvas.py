...
# File: /home/ywatanabe/proj/scitex-code/examples/vis/demo_canvas.py

# Timestamp: 2025-12-08
"""
Demo: scitex.vis - Canvas-Based Figure Composition

Demonstrates composing publication figures from multiple panels.

API:
    canvas = stx.vis.Canvas("fig1", width_mm=180, height_mm=150)
...
def create_sample_plots(output_dir: Path) -> dict:
    """Create sample plots for panels."""
    paths = {}
    ...
    # Bar chart
    fig, ax = stx.plt.subplots()
    ax.bar(range(5), [23, 45, 56, 78, 32])
    ax.set_xticks(range(5))
    ax.set_xticklabels(["A", "B", "C", "D", "E"])
    ax.set_xyt(x="Category", y="Value", t="Category Comparison")
    ...
    ax.scatter(x, x + np.random.randn(50) * 0.5, alpha=0.6)
    ax.set_xyt(x="X Variable", y="Y Variable", t="Correlation Plot")
    ...
    # Histogram
    fig, ax = stx.plt.subplots()
    ax.hist(np.random.randn(1000), bins=30, alpha=0.7, edgecolor="black")
    ax.set_xyt(x="Value", y="Frequency", t="Distribution")
...
def demo_basic_canvas(CONFIG, paths):
    ...
    canvas = stx.vis.Canvas("fig1_demo", width_mm=180, height_mm=150)
...
def demo_panel_transforms(CONFIG, paths):
    ...
    canvas = stx.vis.Canvas("fig2_transforms", width_mm=180, height_mm=130)
...
def demo_update_panel(CONFIG, paths):
    ...
    canvas = stx.vis.Canvas("fig3_update", width_mm=180, height_mm=130)
    canvas.add_panel(
        "panel_a", paths["trig"], position=(10, 10), size=(80, 60), label="A"
    )
...
def demo_symlink_canvas(CONFIG, paths):
    ...
    canvas = stx.vis.Canvas("fig4_symlink", width_mm=180, height_mm=120)
    canvas.add_panel(
...
def demo_bundle_canvas(CONFIG, paths):
    ...
    canvas = stx.vis.Canvas("fig5_bundle", width_mm=180, height_mm=120)
    canvas.add_panel(
        "panel_a", paths["trig"], position=(10, 10), size=(160, 100), label="A"
    )
...
def demo_canvas_caption(CONFIG, paths):
    ...
    canvas = stx.vis.Canvas("fig6_caption", width_mm=180, height_mm=150)
...
def demo_copy_canvas(CONFIG, paths):
    ...
    canvas = stx.vis.Canvas("fig7_original", width_mm=180, height_mm=120)
    canvas.add_panel(
        "panel_a", paths["trig"], position=(10, 10), size=(160, 100), label="A"
    )
...
def main(
    ...
    out = Path(CONFIG.SDIR_OUT)
    paths = create_sample_plots(out / "plots")
