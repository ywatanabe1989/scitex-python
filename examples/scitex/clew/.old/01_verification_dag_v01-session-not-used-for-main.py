...
Usage:
    ...
    python 01_verification_dag.py --skip-failure  # Skip failure demonstration
    python 01_verification_dag.py --skip-rerun    # Skip rerun verification demo
...
def break_chain():
    ...
    # Only modify clean_C.csv to cause hash mismatch
    # This leaves branch A and B intact to show ✓ and ✓✓
    clean_c = out_dir / "clean_C.csv"
    if clean_c.exists():
...
def run_pipeline():
    ...
    print("=" * 70)
    print("Running Complex Patterns Pipeline (3 branches merging)")
    print("=" * 70)
...
def verify_by_rerun():
    ...
    Note: Full rerun verification would re-execute scripts, but for multi-step
    pipelines this requires more sophisticated orchestration. Here we verify
    outputs match stored hashes and record the result as rerun-verified.
    """
    ...
    for run in runs:
        ...
        if any(
            ...
            # Verify current state matches stored hashes
            result = verify_run(sid)
            if result.is_verified:
                # Record as rerun-verified (simulates successful script re-execution)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Complex verification patterns demo")
    parser.add_argument(
        "--skip-failure", action="store_true", help="Skip failure demonstration"
    )
    parser.add_argument(
        "--skip-rerun", action="store_true", help="Skip rerun verification demo"
    )
    ...
    #    This only modifies branch C files
