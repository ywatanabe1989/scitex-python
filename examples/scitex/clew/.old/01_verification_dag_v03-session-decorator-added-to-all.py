...
def _verify_by_rerun(n_sessions: int = 2):
    """Record rerun verification for sessions to demonstrate ✓✓ badge.

    Marks the first N verified sessions that won't be affected by _break_chain().
    Sessions outputting clean_C.csv or downstream files are excluded.
    """
    ...
    # Files that will be affected by _break_chain() modification
    affected_files = {"clean_C.csv", "final.csv", "stats_by_source.csv", "report.json"}

    verified_count = 0
    for run in runs:
        sid = run["session_id"]
        outputs = db.get_file_hashes(sid, role="output")
        output_names = {Path(p).name for p in outputs.keys()}

        # Skip sessions whose outputs will be affected by _break_chain()
        if output_names & affected_files:
            continue
        ...
        if result.is_verified:
            ...
            print(f"  ✓✓ Recorded rerun verification: {list(output_names)}")
            verified_count += 1
            if verified_count >= n_sessions:
                break
