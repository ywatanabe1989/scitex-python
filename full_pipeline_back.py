#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-08-07 02:06:57 (ywatanabe)"
# File: /home/ywatanabe/proj/scitex_repo/full_pipeline.py
# ----------------------------------------
...
async def download_with_zotero_translators():
    ...
    downloads_dir = Path(os.getenv("HOME")) / "downloads"
    ...
    try:
        ...
        if aaas_button_index >= 0:
            ...
            print(
                f'Found {len(pdf_info["mainArticleLinks"])} main article PDF links'
            )
            print(
                f'Found {len(pdf_info["highConfidenceLinks"])} high confidence PDF links'
            )

            # Show the most promising links
            main_links = pdf_info["mainArticleLinks"][
                :3
            ]  # Top 3 main article links
            for i, link in enumerate(main_links):
                print(
                    f'  [{i}] "{link["text"][:40]}..." (confidence: {link["confidence"]})'
                )
            ...
            if main_links:
                ...
                print(
                    f'Using: "{best_link["text"][:40]}..." (confidence: {best_link["confidence"]})'
                )
                ...
                try:
                    ...
                    if best_link["href"] and ".pdf" in best_link["href"]:
                        ...
                        if download_path.exists():
                            size_mb = download_path.stat().st_size / (
                                1024 * 1024
                            )
                            print(
                                f'🎉 SUCCESS! PDF downloaded via direct URL: {paper["filename"]} ({size_mb:.1f} MB)'
                            )
                        ...
                        if "clicked" in click_result:
                            ...
                            if download_path.exists():
                                ...
                                print(
                                    f'🎉 SUCCESS! PDF downloaded via click: {paper["filename"]} ({size_mb:.1f} MB)'
                                )
                        else:
                ...
                print(
                    "❌ No suitable PDF links found with Zotero-style detection"
                )

            # Take final screenshot for analysis
            await popup.screenshot(
                path="zotero_style_detection.png", full_page=True
            )
...
if __name__ == "__main__":
    ...
    os.environ["SCITEX_SCHOLAR_OPENURL_RESOLVER_URL"] = (
        "https://unimelb.hosted.exlibrisgroup.com/sfxlcl41"
    )
    os.environ["SCITEX_SCHOLAR_ZOTERO_TRANSLATORS_DIR"] = (
        "/home/ywatanabe/proj/SciTeX-Code/src/scitex/scholar/zotero_translators/"
    )
