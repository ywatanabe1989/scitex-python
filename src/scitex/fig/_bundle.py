...
"""
Figz - Object-oriented API for .figz bundles.
...
class Figz:
    ...
    def add_panel(
        ...
        self._spec["panels"] = [p for p in self._spec.get("panels", []) if p["id"] != panel_id]
    ...
    def update_panel_position(
        self, panel_id: str, x_mm: float, y_mm: float
    ) -> None:
    ...
    def render_preview(self, format: str = "png", dpi: int = 150) -> bytes:
        ...
        size = self.size_mm
        fig, ax = plt.subplots(figsize=(size.get("width", 170) / 25.4, size.get("height", 120) / 25.4))
        ...
        for panel in self.panels:
            ...
            try:
                ...
                ax.imshow(img, extent=[pos.get("x_mm", 0), pos.get("x_mm", 0) + sz.get("width_mm", 80),
                                       pos.get("y_mm", 0) + sz.get("height_mm", 60), pos.get("y_mm", 0)])
            finally:
