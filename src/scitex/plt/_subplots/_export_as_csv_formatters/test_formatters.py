...
class FormattersTest(unittest.TestCase):
    ...
    def test_format_plot_ecdf(self):
        ...
        tracked_dict = {
            "ecdf_df": pd.DataFrame(
                {"x": np.linspace(-3, 3, 100), "ecdf": np.linspace(0, 1, 100)}
            )
        }
        id = "test_ecdf"
    ...
    def test_format_plot_heatmap(self):
        ...
        tracked_dict = {"data": data, "x_labels": x_labels, "y_labels": y_labels}
        id = "test_heatmap"
    ...
    def test_format_plot_violin(self):
        ...
        tracked_dict = {"data": data, "labels": labels}
        id = "test_violin"
        ...
        tracked_dict = {"data": data_df, "x": "group", "y": "values"}
        df = _format_plot_violin(id, tracked_dict, {})
        self.assertFalse(df.empty)
    ...
    def test_format_plot_scatter_hist(self):
        ...
        self.assertFalse(df.empty)
        self.assertTrue(
            any(col.startswith(f"{id}_scatter_hist_x") for col in df.columns)
        )
        self.assertTrue(
            any(col.startswith(f"{id}_scatter_hist_y") for col in df.columns)
        )
