#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-07-30 09:14:45 (ywatanabe)"
# File: /home/ywatanabe/proj/scitex_repo/src/scitex/scholar/open_url/_OpenURLResolver_v11-manual.py
# ----------------------------------------
from __future__ import annotations
import os
__FILE__ = (
    "./src/scitex/scholar/open_url/_OpenURLResolver_v11-manual.py"
)
...
class OpenURLResolver:
    ...
    def _is_publisher_url(self, url: str, doi: str = "") -> bool:
        ...
        return any(domain in url.lower() for domain in self.PUBLISHER_DOMAINS)
    ...
    def _resolve_single(
        ...
        try:
            ...
            self._print_success(doi, result)
            return result
    ...
    async def _resolve_parallel_async(
        ...
        """
        Resolves a list of DOIs in parallel.
        ...
        logger.info(
            f"--- Starting parallel resolution for {len(dois)} DOIs ---"
        )

        # Create a list of concurrent tasks for each DOI
        tasks = [self._resolve_single_async(doi=doi) for doi in dois]
    ...
    def resolve(self, dois: Union[str, List[str]]) -> Optional[Dict[str, Any]]:
        """Synchronous wrapper for _resolve_parallel_async."""
        loop = asyncio.new_event_loop()
        try:
            results = loop.run_until_complete(
                self._resolve_parallel_async(dois)
            )
    ...
    def _print_success(self, doi, result):
        if (
            result
