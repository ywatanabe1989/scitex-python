...
from ..browser._BrowserConfig import get_browser_config, BrowserMode, BrowserConfiguration
...
class OpenURLResolver:
    ...
    def __init__(
        ...
        if config:
            ...
            logger.info(f"🔧 Using centralized config for mode: {browser_mode.value}")
        else:
            ...
            logger.info(f"🔧 Using legacy config converted to: {self.browser_config}")
        ...
        if self.zenrows_api_key:
            ...
            logger.info(f"🔧 Creating BrowserManager with centralized config: {self.browser_config}")
        ...
        if self.capture_screenshots:
            ...
            self.screenshot_dir = scholar_paths.get_screenshots_dir() / "openurl"
            self.screenshot_dir.mkdir(parents=True, exist_ok=True)
            self.session_timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    async def _capture_checkpoint_screenshot(self, page, stage: str, doi: str = "") -> Optional[str]:
        ...
        try:
            ...
            doi_safe = doi.replace("/", "-").replace(".", "_") if doi else "unknown"
            ...
            logger.info(f"📸 Screenshot captured: {stage} -> {screenshot_name}")
    ...
    async def _resolve_single_async(
        ...
        try:
            ...
            await self._capture_checkpoint_screenshot(page, "01_openurl_loaded", doi)
            ...
            await self._capture_checkpoint_screenshot(page, "02_stealth_applied", doi)
