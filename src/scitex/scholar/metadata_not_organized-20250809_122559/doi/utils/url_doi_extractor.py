...
# File: /home/ywatanabe/proj/scitex_repo/src/scitex/scholar/doi/utils/url_doi_extractor.py
# ----------------------------------------
from __future__ import annotations
import os
__FILE__ = (
    "./src/scitex/scholar/doi/utils/url_doi_extractor.py"
)
__DIR__ = os.path.dirname(__FILE__)
# ----------------------------------------

"""
URLDOISource: Extract DOIs directly from URL fields in BibTeX entries.
...
class URLDOISource:
    ...
    def extract_from_bibtex_entry(self, entry: Dict) -> Optional[str]:
        ...
        if url:
            ...
            if extracted_doi:
                logger.info(f"Extracted DOI from URL for '{entry.get('title', 'Unknown')}': {extracted_doi}")
    ...
    def extract_from_bibtex_entries(self, entries: List[Dict]) -> Dict[str, str]:
        ...
        if results:
            logger.info(f"URLDOISource: Extracted DOIs from {len(results)} entries")
...
def main():
    """Test and demonstrate URLDOISource functionality."""
    print("=" * 60)
    print("URLDOISource Test Suite")
    print("=" * 60)
    
    extractor = URLDOISource()
    ...
    for i, entry in enumerate(test_entries):
        result = extractor.extract_from_bibtex_entry(entry)
        status = "✅" if result else "❌"
        print(f"   {status} Entry {i}: '{entry['title']}' → {result}")
        
    # Test batch extraction
    print("\n3. Testing batch extraction:")
    batch_results = extractor.extract_from_bibtex_entries(test_entries)
    print(f"   📊 Extracted DOIs from {len(batch_results)} out of {len(test_entries)} entries")
    ...
    print("\n" + "=" * 60)
    print("✅ URLDOISource test completed!")
    print("=" * 60)
    print("\nUsage patterns:")
    print("1. Single URL: extractor.extract_doi_from_url(url)")
    print("2. BibTeX entry: extractor.extract_from_bibtex_entry(entry)")
    print("3. Batch entries: extractor.extract_from_bibtex_entries(entries)")
