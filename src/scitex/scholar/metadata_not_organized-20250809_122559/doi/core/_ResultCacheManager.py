#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-08-05 00:00:00 (ywatanabe)"
# File: /home/ywatanabe/proj/SciTeX-Code/src/scitex/scholar/doi/core/_ResultCacheManager.py
# ----------------------------------------
...
class ResultCacheManager:
    ...
    def __init__(self, config, project: str = "master"):
        ...
        logger.debug(f"ResultCacheManager initialized for project: {project}")

    def check_scholar_library(self, title: str, year: Optional[int] = None) -> Optional[str]:
                    ...
                    if metadata_file.exists():
                        try:
                            with open(metadata_file, 'r') as f:
                                ...
                                year_match = (year is None or stored_year is None or 
                                            stored_year == year)
        ...
        try:
            ...
            comprehensive_metadata = {
                **existing_metadata,  # Preserve existing data
                'title': title,
                'title_source': bibtex_source if bibtex_source else 'doi_resolver',
                'year': paper_info.get('year'),
                'year_source': 'doi_source' if (metadata and metadata.get('year')) else (bibtex_source if bibtex_source and year else 'doi_resolver'),
                'authors': paper_info.get('authors', []),
                'authors_source': 'doi_source' if (metadata and metadata.get('authors')) else (bibtex_source if bibtex_source and authors else 'doi_resolver'),
                'doi': doi,
                'doi_source': source,
            ...
            if paper_info.get('journal'):
                ...
                comprehensive_metadata['journal_source'] = 'doi_source' if (metadata and metadata.get('journal')) else (bibtex_source if bibtex_source else 'doi_resolver')
            
            # Add any additional metadata from DOI source
            if metadata:
                for field, value in metadata.items():
                    if field not in ['title', 'year', 'authors', 'journal', 'doi'] and value is not None:
                        comprehensive_metadata[field] = value
                        comprehensive_metadata[f'{field}_source'] = 'doi_source'
            
            # Save comprehensive metadata
            with open(master_metadata_file, 'w') as f:
                json.dump(comprehensive_metadata, f, indent=2)
            
            logger.success(f"Saved to master Scholar library: {paper_id} ({doi})")
    ...
    def save_unresolved_entry(
        ...
        try:
            ...
            unresolved_metadata = {
                ...
                'authors': authors or [],
                'authors_source': bibtex_source if bibtex_source and authors else 'input',
                'doi': None,
                'doi_source': None,
    ...
    def _create_project_symlink(self, paper_id: str, readable_name: str) -> bool:
        ...
        try:
            # Get paths
            master_paper_dir = self.config.path_manager.get_collection_dir("master") / paper_id
            project_dir = self.config.path_manager.get_collection_dir(self.project)
            project_readable_dir = project_dir.parent / f"{self.project}-human-readable"
            
            # Ensure directories exist
            project_readable_dir.mkdir(parents=True, exist_ok=True)
            
            # Create symlink
            symlink_path = project_readable_dir / readable_name
            if not symlink_path.exists():
                # Create relative symlink to master paper directory
                relative_target = Path("..") / "master" / paper_id
                symlink_path.symlink_to(relative_target)
                logger.debug(f"Created project symlink: {symlink_path} -> {relative_target}")
                return True
            
            return True
            
        except Exception as e:
            logger.error(f"Error creating project symlink: {e}")
            return False
    ...
    def _generate_readable_name(self, title: str, year: Optional[int], authors: Optional[List[str]]) -> str:
        ...
        if title:
            ...
            clean_title = "_".join(word.strip().replace(" ", "_") for word in title_words if word.strip())
        else:
    ...
    def get_unresolved_entries(self, project_name: Optional[str] = None) -> List[Dict]:
        ...
        try:
            ...
            collection_dir = self.config.path_manager.get_collection_dir(collection_name)
                    ...
                    if metadata_file.exists():
                        try:
                            with open(metadata_file, 'r') as f:
                                ...
                                if (metadata.get('doi_resolution_failed') or 
                                    metadata.get('resolution_status') == 'unresolved' or
                                    not metadata.get('doi')):
                                    ...
                                    unresolved_entries.append({
                                        ...
                                        'last_attempt': metadata.get('doi_last_attempt'),
            ...
            logger.info(f"Found {len(unresolved_entries)} unresolved entries in {collection_name}")
    ...
    def copy_bibtex_to_library(self, bibtex_path: str, project_name: Optional[str] = None) -> str:
        ...
        try:
            ...
            bibtex_dest = collection_dir / f"source_{bibtex_source.name}"
    ...
    def get_cache_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not master_dir.exists():
                return {"total_papers": 0, "resolved_papers": 0, "unresolved_papers": 0}
            ...
            return {
                ...
                "resolution_rate": resolved_papers / total_papers if total_papers > 0 else 0
            }
...
if __name__ == "__main__":
    ...
    print("ResultCacheManager Test:")
    
    # This would require a real config in practice
    # manager = ResultCacheManager(config, "test_project")
    
    print("Note: Full testing requires ScholarConfig instance")
    print("Core functionality:")
    print("- check_scholar_library(): Search for existing DOIs")
