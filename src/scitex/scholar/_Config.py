#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-07-31 00:50:56 (ywatanabe)"
...
class ScholarConfig:
    ...
    pubmed_email: str = field(
        default_factory=lambda: os.getenv(
            "SCITEX_SCHOLAR_PUBMED_EMAIL", "research@example.com"
        )
    )
    crossref_email: str = field(
        default_factory=lambda: os.getenv(
            "SCITEX_SCHOLAR_CROSSREF_EMAIL", "research@example.com"
        )
    )
    ...
    enable_auto_enrich: bool = field(
        default_factory=lambda: os.getenv(
            "SCITEX_SCHOLAR_AUTO_ENRICH", "true"
        ).lower()
        == "true"
    )
    use_impact_factor_package: bool = field(
        default_factory=lambda: os.getenv(
            "SCITEX_SCHOLAR_USE_IMPACT_FACTOR_PACKAGE", "true"
        ).lower()
        == "true"
    )
    enable_auto_download: bool = field(
        default_factory=lambda: os.getenv(
            "SCITEX_SCHOLAR_AUTO_DOWNLOAD", "false"
        ).lower()
        == "true"
    )
    ...
    google_scholar_timeout: int = field(
        default_factory=lambda: int(
            os.getenv("SCITEX_SCHOLAR_GOOGLE_SCHOLAR_TIMEOUT", "10")
        )
    )
    ...
    openathens_enabled: bool = field(
        default_factory=lambda: os.getenv(
            "SCITEX_SCHOLAR_OPENATHENS_ENABLED", "false"
        ).lower()
        == "true"
    )
    ...
    openathens_username: Optional[str] = field(
        default_factory=lambda: os.getenv("SCITEX_SCHOLAR_OPENATHENS_USERNAME")
    )
    openathens_password: Optional[str] = field(
        default_factory=lambda: os.getenv("SCITEX_SCHOLAR_OPENATHENS_PASSWORD")
    )
    openathens_institution_name: Optional[str] = field(
        default_factory=lambda: os.getenv(
            "SCITEX_SCHOLAR_OPENATHENS_INSTITUTION_NAME"
        )
    )
    ...
    use_lean_library: bool = field(
        default_factory=lambda: os.getenv(
            "SCITEX_SCHOLAR_USE_LEAN_LIBRARY", "true"
        ).lower()
        == "true"
    )
    lean_library_browser_profile: Optional[str] = field(
        default_factory=lambda: os.getenv(
            "SCITEX_SCHOLAR_LEAN_LIBRARY_BROWSER_PROFILE"
        )
    )
    user_agent: str = "SciTeX-Scholar/1.0"
    ...
    zenrows_proxy_username: Optional[str] = field(
        default_factory=lambda: os.getenv(
            "SCITEX_SCHOLAR_ZENROWS_PROXY_USERNAME"
        )
    )
    zenrows_proxy_password: Optional[str] = field(
        default_factory=lambda: os.getenv(
            "SCITEX_SCHOLAR_ZENROWS_PROXY_PASSWORD"
        )
    )
    zenrows_proxy_domain: str = field(
        default_factory=lambda: os.getenv(
            "SCITEX_SCHOLAR_ZENROWS_PROXY_DOMAIN", "superproxy.zenrows.com"
        )
    )
    zenrows_proxy_port: str = field(
        default_factory=lambda: os.getenv(
    ...
    zotero_library_id: Optional[str] = field(
        default_factory=lambda: os.getenv("SCITEX_SCHOLAR_ZOTERO_LIBRARY_ID")
    )
    zotero_api_key: Optional[str] = field(
        default_factory=lambda: os.getenv("SCITEX_SCHOLAR_ZOTERO_API_KEY")
    )
    zotero_library_type: str = field(
        default_factory=lambda: os.getenv(
            "SCITEX_SCHOLAR_ZOTERO_LIBRARY_TYPE", "user"
        )
    )
    
    # 2Captcha configuration for CAPTCHA solving
    twocaptcha_api_key: Optional[str] = field(
        default_factory=lambda: os.getenv("SCITEX_SCHOLAR_2CAPTCHA_API_KEY")
    )
    enable_captcha_solving: bool = field(
        default_factory=lambda: os.getenv(
    ...
    browser_backend: str = field(
        default_factory=lambda: os.getenv("SCITEX_SCHOLAR_BROWSER_BACKEND", "local")
    )
    zenrows_proxy_country: str = field(
        default_factory=lambda: os.getenv("SCITEX_SCHOLAR_ZENROWS_PROXY_COUNTRY", "us")
    )
    ...
    def create_browser_manager(self, auth_manager=None):
        """Create a BrowserManager with the configured backend.
        
        Returns:
            BrowserManager instance configured with the appropriate backend
        """
        from .browser._BrowserManager import BrowserManager
        
        return BrowserManager(
            auth_manager=auth_manager,
            backend=self.browser_backend,
            zenrows_api_key=self.zenrows_api_key,
            proxy_country=self.zenrows_proxy_country,
        )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "zenrows_api_key": self.zenrows_api_key,
            "zenrows_proxy_username": self.zenrows_proxy_username,
            "zenrows_proxy_password": self.zenrows_proxy_password,
            "zenrows_proxy_domain": self.zenrows_proxy_domain,
            "zenrows_proxy_port": self.zenrows_proxy_port,
    ...
    def show_env_vars(cls) -> str:
        ...
        env_vars = {
            ...
            "SCITEX_SCHOLAR_ZENROWS_API_KEY": "ZenRows API key for web scraping",
            "SCITEX_SCHOLAR_ZENROWS_PROXY_USERNAME": "ZenRows proxy username",
            "SCITEX_SCHOLAR_ZENROWS_PROXY_PASSWORD": "ZenRows proxy password",
            "SCITEX_SCHOLAR_ZENROWS_PROXY_DOMAIN": "ZenRows proxy domain (default: superproxy.zenrows.com)",
            "SCITEX_SCHOLAR_ZENROWS_PROXY_PORT": "ZenRows proxy port (default: 1337)",
            "SCITEX_SCHOLAR_ZOTERO_LIBRARY_ID": "Zotero library ID",
            "SCITEX_SCHOLAR_ZOTERO_API_KEY": "Zotero API key",
    ...
    def show_config(self) -> str:
        ...
        field_to_env = {
            ...
            "zenrows_api_key": "SCITEX_SCHOLAR_ZENROWS_API_KEY",
            "zenrows_proxy_username": "SCITEX_SCHOLAR_ZENROWS_PROXY_USERNAME",
            "zenrows_proxy_password": "SCITEX_SCHOLAR_ZENROWS_PROXY_PASSWORD",
            "zenrows_proxy_domain": "SCITEX_SCHOLAR_ZENROWS_PROXY_DOMAIN",
            "zenrows_proxy_port": "SCITEX_SCHOLAR_ZENROWS_PROXY_PORT",
