...
from ..open_url._ResolverLinkFinder import ResolverLinkFinder, find_and_click_resolver_link_async
...
class PDFDownloader:
    ...
    async def _download_async_from_doi_async(
        ...
        # Resolve DOI to URL
        resolved_url = await self._resolve_doi_async_async(doi)
        if not resolved_url:
        ...
        # Step 1: Get authenticate_async session (if available)
        auth_session = await self._get_authenticate_async_session_async()
        if auth_session:
            provider = auth_session.get('context', {}).get('provider', 'Unknown')
            logger.info(f"Using authenticate_async session from {provider}")
        else:
        ...
        if auth_session:
            if self.use_openathens and self.openathens_authenticator:
                # OpenAthens authenticate_async strategies
                strategies = [
                ...
                # EZProxy authenticate_async strategies
                strategies = [
                    ("ZenRows", self._try_zenrows_async),  # Primary - anti-bot bypass
                ...
                # Shibboleth authenticate_async strategies
                strategies = [
                    ("ZenRows", self._try_zenrows_async),  # Primary - anti-bot bypass
                ...
                strategies = [
                    ("ZenRows", self._try_zenrows_async),  # Primary - anti-bot bypass
                    ("Lean Library", self._try_lean_library_async),  # Browser extension
                    ("OpenURL Resolver", self._try_openurl_async_resolver_async),  # Institutional access
    ...
    async def _get_authenticate_async_session_async(self) -> Optional[Dict[str, Any]]:
        """
        Get authenticate_async session from available authentication providers.
        
        Returns session data with cookies/headers if authenticate_async,
        None otherwise.
        """
        # Check EZProxy if available (higher priority)
        if self.ezproxy_authenticator:
            try:
                if await self.ezproxy_authenticator.is_authenticate_async():
                    cookies = await self.ezproxy_authenticator.get_auth_cookies_async()
                    return {
                        'cookies': cookies,
                        'headers': await self.ezproxy_authenticator.get_auth_headers_async(),
                        'context': {
                            'provider': 'EZProxy',
        ...
        if self.shibboleth_authenticator:
            try:
                if await self.shibboleth_authenticator.is_authenticate_async():
                    cookies = await self.shibboleth_authenticator.get_auth_cookies_async()
                    return {
                        'cookies': cookies,
                        'headers': await self.shibboleth_authenticator.get_auth_headers_async(),
                        'context': {
                            'provider': 'Shibboleth',
        ...
        if self.use_openathens and self.openathens_authenticator:
            try:
                # Reload session cache in case another process authenticate_async
                await self.openathens_authenticator._load_session_cache()
                
                if await self.openathens_authenticator.is_authenticate_async():
    ...
    async def _try_zotero_translator_async(
        ...
        # If we have auth session, run translator with authenticate_async browser
        if auth_session and auth_session.get('cookies'):
            ...
            for resolved_url in result['full_text_urls']:
                ...
                async with async_playwright() as p:
                    ...
                    try:
                        ...
                        final_url_after_click = await find_and_click_resolver_link_async(page, doi)
    ...
    async def _try_playwright_async(
        ...
        if self.zenrows_stealth_browser:
            ...
            try:
                browser = await self.zenrows_stealth_browser.get_browser_async()
            ...
            try:
                ...
                # Look for PDF links (enhanced for authenticate_async pages)
    ...
    def _get_publisher_pdf_urls(self, url: str, doi: str) -> List[str]:
        ...
        patterns = {
            ...
            "cell.com": [
                ...
                lambda: url.replace("/article/", "/action/show_asyncPdf?pii="),
            ],
            "sciencedirect.com": [
    ...
    async def _resolve_doi_async_async(self, doi: str) -> Optional[str]:
    ...
    async def batch_download_async(
        ...
        if progress_callback is None and show_async_progress:
            progress_tracker = create_progress_tracker(
                total, show_async_progress=True
            )
    ...
    async def _download_async_file_with_auth_async(
        ...
        """Download file using authenticate_async session."""
    ...
    async def _run_translator_with_auth_async(
        ...
        """Run Zotero translator with authenticate_async browser context."""
        from playwright.async_api import async_playwright
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                # Also look for PDF links on authenticate_async page
