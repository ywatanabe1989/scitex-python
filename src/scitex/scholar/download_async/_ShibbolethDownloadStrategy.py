...
"""
Shibboleth-based download_async strategy for PDFs.

This strategy uses Shibboleth authentication to download_async PDFs
...
class ShibbolethDownloadStrategy(BaseDownloadStrategy):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize Shibboleth download_async strategy.
    ...
    async def can_download_async(self, url: str, paper: Optional[Any] = None) -> bool:
        """
        Check if this strategy can download_async from the given URL.
    ...
    async def download_async(
        ...
        try:
            ...
            try:
                ...
                # Set up download_async handling
                download_async_path = output_path.parent
                download_async_path.mkdir(parents=True, exist_ok=True)
                ...
                # Look for PDF download_async link on the page
                pdf_link = await self._find_pdf_link_async(page)
                
                if pdf_link:
                    # Download the PDF
                    async with page.expect_download_async(timeout=self.timeout * 1000) as download_async_info:
    ...
    async def _find_pdf_link_async(self, page: Page) -> Optional[str]:
        """
        Find PDF download_async link on the page.
        ...
        pdf_selectors = [
            ...
            'a[class*="pdf-link"]',
            'a[class*="download_async-pdf"]',
            'a[data-track-action="download_async pdf"]',
            # Publisher-specific
            'a[data-article-pdf]',  # Nature
            'a.article-pdf-download_async',  # Various publishers
            'a[rel="alternate"][type="application/pdf"]',
            # JSTOR specific
            'a.pdfLink',
            'a[data-qa="download_async-pdf"]',
            # Project MUSE specific
            'a.download_async-link',
            # IEEE specific
            'a[aria-label*="PDF"]',
        ]
