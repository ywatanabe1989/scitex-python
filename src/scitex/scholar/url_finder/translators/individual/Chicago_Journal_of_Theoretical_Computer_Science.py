...
class ChicagoJournalOfTheoreticalComputerScienceTranslator:
    ...
    SINGLE_RE = re.compile(r'^https?://cjtcs\.cs\.uchicago\.edu/articles/\w+/\d+/contents\.html')
    MULTIPLE_RE = re.compile(r'^https?://cjtcs\.cs\.uchicago\.edu/articles/\w+/contents\.html')
    ...
    async def do_web(self, doc: BeautifulSoup, url: str, request_text_func) -> Dict[str, Any]:
        """Extract article data."""
        return await self.scrape(doc, url, request_text_func)

    async def scrape(self, doc: BeautifulSoup, url: str, request_text_func) -> Dict[str, Any]:
        """
    ...
    def _parse_bibtex(self, bib_text: str, item: Dict[str, Any]):
        ...
        # Extract title
        title_match = re.search(r'title\s*=\s*[{"](.*?)[}"]', bib_text, re.IGNORECASE | re.DOTALL)
        if title_match:
            item['title'] = title_match.group(1).strip()

        # Extract authors
        author_match = re.search(r'author\s*=\s*[{"](.*?)[}"]', bib_text, re.IGNORECASE | re.DOTALL)
        if author_match:
        ...
        issue_match = re.search(r'(?:issue|number)\s*=\s*[{"](.*?)[}"]', bib_text, re.IGNORECASE)
        if issue_match:
            item['issue'] = issue_match.group(1).strip()

        # Extract journal
        journal_match = re.search(r'journal\s*=\s*[{"](.*?)[}"]', bib_text, re.IGNORECASE)
        if journal_match:
