...
class BioconductorTranslator:
    ...
    def detect_web(self, doc: BeautifulSoup, url: str) -> str:
        ...
        elif '/search/index.html' in url and self._get_search_results(doc, check_only=True):
            return 'multiple'
        return ''
    ...
    def scrape(self, doc: BeautifulSoup, url: str) -> Dict[str, Any]:
        ...
        version_cell = doc.select_one('table tbody tr td:-soup-contains("Version") + td')
        if version_cell:
            item['versionNumber'] = version_cell.get_text(strip=True)

        # Extract license
        license_cell = doc.select_one('table tbody tr td:-soup-contains("License") + td')
        if license_cell:
            item['rights'] = license_cell.get_text(strip=True)

        # Extract short URL
        url_cell = doc.select_one('table tbody tr td:-soup-contains("Package Short Url") + td')
        if url_cell:
