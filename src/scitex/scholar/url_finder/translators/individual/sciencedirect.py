...
class ScienceDirectTranslator(BaseTranslator):
    ...
    async def extract_pdf_urls_async(cls, page: Page) -> List[str]:
        ...
        try:
            ...
            pass
        try:
            if await page.locator('.access-options-link-text').first.is_visible(timeout=1000):
                return []
        except:
            pass
        try:
            if await page.locator('#check-access-popover').first.is_visible(timeout=1000):
                return []
        except:
            pass
        ...
        try:
            pdf_url = await page.locator('#pdfLink').first.get_attribute('href', timeout=2000)
        ...
        try:
            pdf_url = await page.locator('[name="citation_pdf_url"]').first.get_attribute('content', timeout=2000)
            if pdf_url:
                return [pdf_url]
        except:
            pass
        ...
        try:
            intermediate_url = await page.locator('.PdfEmbed > object').first.get_attribute('data', timeout=2000)
            if intermediate_url:
        ...
        try:
            json_script = await page.locator('script[type="application/json"]').first.text_content(timeout=2000)
            if json_script:
                import json
                data = json.loads(json_script)

                # Try to construct PDF URL from JSON
                url_metadata = data.get('article', {}).get('pdfDownload', {}).get('urlMetadata', {})
        ...
        try:
            canonical_url = await page.locator('link[rel="canonical"]').first.get_attribute('href', timeout=2000)
            if canonical_url:
