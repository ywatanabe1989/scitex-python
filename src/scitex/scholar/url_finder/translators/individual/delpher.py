...
class DelpherTranslator:
    ...
    def _get_metadata_field(self, details: BeautifulSoup, field_name: str) -> Optional[str]:
        """Extract metadata field value from details section."""
        dt = details.find('dt', class_='metadata__details-text', string=lambda x: x and field_name in x)
        if dt:
    ...
    def scrape(self, doc: BeautifulSoup, url: str) -> Dict[str, Any]:
        ...
        # Extract title
        title = self._get_metadata_field(details, 'Titel') or self._get_metadata_field(details, 'Kop')
        if not title:
        ...
        # Extract date
        date = self._get_metadata_field(details, 'Publicatiedatum') or self._get_metadata_field(details, 'Datum')
        if not date:
        ...
        item['libraryCatalog'] = self._get_metadata_field(details, 'Herkomst') or 'Delpher'
        item['publisher'] = self._get_metadata_field(details, 'Drukker/Uitgever') or self._get_metadata_field(details, 'Uitgever')
        ...
        # Extract authors
        author_dts = details.find_all('dt', class_='metadata__details-text',
        ...
        url_input = doc.select_one('input.object-view-menu__share-links-details-input:last-of-type')
