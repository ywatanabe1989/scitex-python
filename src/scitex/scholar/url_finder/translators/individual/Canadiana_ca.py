...
class CanadianaCATranslator:
    ...
    def scrape(self, doc: BeautifulSoup, url: str) -> Dict[str, Any]:
        ...
        if 'Published' in data_tags or 'Imprint' in data_tags:
            ...
            if imprint:
                ...
                if place_match:
                    item['place'] = place_match.group(0).replace(':', '').replace('[', '').replace(']', '').strip()

                # Extract publisher
                publisher_match = re.search(r':[^,\d]+', imprint)
                if publisher_match:
                    item['publisher'] = publisher_match.group(0).replace(':', '').replace('[', '').replace(']', '').replace('?', '').strip()
    ...
    def _associate_data(self, item: Dict, data_tags: Dict, field: str, zotero_field: str):
