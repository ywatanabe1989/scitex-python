...
class CrossrefRESTTranslator(BaseTranslator):
    ...
    @classmethod
    def parse_creators(cls, result: Dict[str, Any], item: Dict[str, Any],
                      type_override_map: Optional[Dict[str, str]] = None) -> None:
    ...
    def process_crossref_result(cls, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if result_type in ['journal', 'journal-article', 'journal-volume', 'journal-issue']:
            item['itemType'] = 'journalArticle'
        elif result_type in ['report', 'report-series', 'report-component']:
            item['itemType'] = 'report'
        elif result_type in ['book', 'book-series', 'book-set', 'book-track',
                            'monograph', 'reference-book', 'edited-book']:
            item['itemType'] = 'book'
        elif result_type in ['book-chapter', 'book-part', 'book-section', 'reference-entry']:
            item['itemType'] = 'bookSection'
            creator_type_override_map = {'author': 'bookAuthor'}
        elif result_type == 'other' and result.get('ISBN') and result.get('container-title'):
            item['itemType'] = 'bookSection'
        ...
        elif result_type in ['proceedings', 'proceedings-article', 'proceedings-series']:
            ...
            if not result.get('author'):
                item['creators'] = [{'lastName': 'Anonymous Reviewer', 'fieldMode': 1, 'creatorType': 'author'}]
    ...
    @classmethod
    async def search_by_doi(cls, doi: str, mailto: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    @classmethod
    async def search_by_query(cls, query: str, mailto: Optional[str] = None) -> List[Dict[str, Any]]:
