...
LII_REGEXP = re.compile(r'^https?://www\.bailii\.org(?:/cgi-bin/markup\.cgi\?doc=)?/\w+/cases/.+\.html')
...
def scrape(doc: Any, url: str) -> Dict[str, Any]:
    ...
    if title_tag:
        ...
        if title_match:
            item["caseName"] = f"{title_match.group(1)} [{title_match.group(2)}] {title_match.group(3)}"
            item["dateDecided"] = f"{title_match.group(4)} {title_match.group(5)} {title_match.group(6)}"
        else:
