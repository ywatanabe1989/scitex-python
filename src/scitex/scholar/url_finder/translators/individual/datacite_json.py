...
class DataciteJSONTranslator:
    ...
    def do_import(self, content: str) -> Dict[str, Any]:
        ...
        if data.get('types'):
            ...
            # Check citeproc type
            if types_info.get('citeproc') and types_info['citeproc'] in self.TYPE_MAPPINGS:
                item_type = self.TYPE_MAPPINGS[types_info['citeproc']]

            # Check for software types
            schema_org = types_info.get('schemaOrg', '').lower()
            if schema_org in ['softwaresourcecode', 'softwareapplication', 'mobileapplication', 'videogame', 'webapplication']:
        ...
        if data.get('types', {}).get('citeproc') == 'dataset' and item_type == 'document':
