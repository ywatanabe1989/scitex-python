...
class CascadillaProceedingsProjectTranslator:
    ...
    def detect_web(self, doc: BeautifulSoup, url: str) -> str:
        ...
        links = doc.select('.pagecontent > .contentpadding > a[href*="abstract"][href$=".html"]')
        if links:
            return 'multiple'
    ...
    def scrape(self, doc: BeautifulSoup, url: str) -> Dict[str, Any]:
        ...
        if pdf_link and pdf_link.get('href'):
            item['attachments'].append({
                'url': pdf_link['href'] if pdf_link['href'].startswith('http') else 'https://www.lingref.com' + pdf_link['href'],
                'title': 'Full Text PDF',
            ...
            for child in bookinfo.children:
                if hasattr(child, 'get_text'):
                    ...
                    if text.startswith('edited by'):
                        ...
                        editors = []
                        for part in editor_text.replace(', and ', '|||').replace(' and ', '|||').split(','):
                        ...
                        for editor_name in editors:
                            item['creators'].append(self._clean_author(editor_name, 'editor'))
                        break
