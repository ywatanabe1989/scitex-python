...
class CBCTranslator:
    ...
    def detect_web(self, doc: BeautifulSoup, url: str) -> str:
        ...
        if any(section in path for section in ['news/', 'sports/', 'radio/', 'books/', 'arts/', 'music/', 'life/', 'television/', 'archives/']):
    ...
    def get_meta_content(self, doc: BeautifulSoup, attribute: str, value: str) -> Optional[str]:
        """
    ...
    def scrape(self, doc: BeautifulSoup, url: str) -> Dict[str, Any]:
        ...
        if ld:
            ...
            if item_type == 'videoRecording' and 'uploadDate' in ld:
                item['date'] = ld['uploadDate'][:10] if len(ld['uploadDate']) >= 10 else ld['uploadDate']
            elif 'datePublished' in ld:
                item['date'] = ld['datePublished'][:10] if len(ld['datePublished']) >= 10 else ld['datePublished']
            ...
            if 'author' in ld and ld['author']:
                authors = ld['author'] if isinstance(ld['author'], list) else [ld['author']]
