...
Run this once to authenticate_async, then use the saved session in your scripts.

Usage:
    python authenticate_async_only.py
...
def main():
    ...
    try:
        scholar = Scholar(
            ...
            enable_auto_download_async=False,  # Don't need download_asyncs for auth
    ...
    try:
        is_auth = scholar.is_openathens_authenticate_async()
        
        if is_auth:
            print("✅ Already authenticate_async! Session is valid.")
            print("\n💡 To force re-authentication, use:")
            print("   scholar.authenticate_async_openathens(force=True)")
            
            # Offer to re-authenticate_async
            response = input("\nRe-authenticate_async anyway? (y/N): ").strip().lower()
            if response != 'y':
                print("\n✅ Using existing session. You're ready to download_async papers!")
                return
            
            print("\n🔄 Force re-authentication...")
            force = True
        else:
            print("❌ Not authenticate_async.")
            force = False
    ...
    try:
        ...
        success = scholar.authenticate_async_openathens(force=force)
        ...
        if success:
            ...
            print("papers.download_async_pdf_asyncs()  # Will use OpenAthens")
            print("```")
