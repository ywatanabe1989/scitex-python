...
"""
Direct download_async of the Nature Neuroscience paper using OpenAthens.
"""
...
async def download_async_paper():
    ...
    if await auth.is_authenticate_async_async():
        print("✅ Using saved OpenAthens session\n")
    else:
        print("❌ No valid session - would need to authenticate_async\n")
        return
    ...
    for i, url in enumerate(urls_to_try, 1):
        ...
        try:
            result = await auth.download_async_with_auth_async(url, output_path)
...
if __name__ == "__main__":
    asyncio.run(download_async_paper())