...
Run with: python quick_test_async_openathens_dois.py
"""
...
async def quick_test_async():
    ...
    try:
        success = await scholar.authenticate_async_openathens()
        if not success:
    ...
    for i, doi in enumerate(TEST_DOIS, 1):
        ...
        try:
            pdf_path = await scholar.download_async_pdf_async(doi, output_dir=output_dir)
    ...
    print(f"\n📊 Results: {len(pdfs)}/{len(TEST_DOIS)} PDFs download_asynced")
