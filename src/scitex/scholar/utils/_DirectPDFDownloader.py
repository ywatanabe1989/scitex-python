...
class DirectPDFDownloader:
    """
    Downloads PDFs by jumping directly to PDF URLs with authenticated browser context.
    ...
    async def download_pdf_direct(self, 
        ...
        Args:
            page: Playwright page with authenticated context
    ...
    async def download_multiple_pdfs(self, 
        ...
        for i, pdf_url in enumerate(pdf_urls):
            try:
                ...
                # Download the PDF
                success, error = await self.download_pdf_direct(page, pdf_url, file_path)
...
async def download_pdfs_direct(page, pdf_urls: List[str], download_dir: Path) -> List[Tuple[str, Path, bool, Optional[str]]]:
