...
class ConfigurationResolver:
    ...
    def resolve_email_configuration(
        ...
        logger.debug(
            f"Resolved email configuration: {list(email_config.keys())}"
        )
        return email_config

    def resolve_sources_configuration(
    ...
    def create_enrichment_configuration(
        self, email_config: Dict[str, str], api_keys: Dict[str, Optional[str]]
    ) -> Dict[str, Any]:
    ...
    def get_configuration_summary(
        ...
        return {
            ...
            "api_keys_configured": [
                k for k, v in api_keys.items() if v is not None
            ],
            "config_file": (
    ...
    def resolve_all_configuration(
        ...
        enrichment_config = self.create_enrichment_configuration(
            email_config, api_keys
        )

        # Validate configuration
        validation = self.validate_configuration(
            email_config, resolved_sources, resolved_project
        )

        # Create summary
        summary = self.get_configuration_summary(
            email_config, resolved_sources, resolved_project, api_keys
        )
