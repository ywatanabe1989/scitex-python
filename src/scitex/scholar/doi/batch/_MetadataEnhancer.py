#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-08-04 23:30:00 (ywatanabe)"
# File: /home/ywatanabe/proj/SciTeX-Code/src/scitex/scholar/doi/batch/_MetadataEnhancer.py
# ----------------------------------------
...
class MetadataEnhancer:
    ...
    def parse_bibtex_authors(self, authors_str: str) -> List[str]:
    ...
    def validate_paper_metadata(self, paper: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if year_str:
            ...
            if parsed_year:
                ...
                if parsed_year < 1900 or parsed_year > 2030:
                    result["warnings"].append(f"Unusual publication year: {parsed_year}")
            else:
        ...
        if authors_str:
            parsed_authors = self.parse_bibtex_authors(authors_str)
    ...
    def enhance_paper_batch(self, papers: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        results["statistics"]["duplicates_found"] = sum(len(group) for group in duplicate_groups.values())
...
if __name__ == "__main__":
    ...
    for i, paper in enumerate(results["enhanced_papers"]):
        ...
        print(f"  Paper {i}: Valid={validation['valid']}, Warnings={len(validation['warnings'])}")
        if validation["warnings"]:
