#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-08-04 23:55:00 (ywatanabe)"
# File: /home/ywatanabe/proj/SciTeX-Code/src/scitex/scholar/doi/core/_SourceManager.py
# ----------------------------------------
...
class SourceManager:
    ...
    def _get_email_for_source(self, name: str) -> str:
        ...
        email_map = {
            ...
            "semantic_scholar_enhanced": "semantic_scholar",
            "arxiv": "arxiv",
        }
    ...
    def _configure_source_specific_settings(self, source_instance: BaseDOISource, name: str):
    ...
    def get_all_sources(self) -> List[BaseDOISource]:
        ...
        """
        return [self.get_source(name) for name in self.sources if self.get_source(name)]
    ...
    def update_email_config(self, email_config: Dict[str, str]):
        ...
        email_dependent_sources = [
            ...
            if name not in ["url_extractor"] and name in self._source_instances
        ]
        
        for name in email_dependent_sources:
            self.reload_source(name)
        
        logger.info(f"Updated email configuration and reloaded {len(email_dependent_sources)} sources")
