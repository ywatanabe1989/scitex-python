#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-08-05 00:05:00 (ywatanabe)"
# File: /home/ywatanabe/proj/SciTeX-Code/src/scitex/scholar/doi/_SingleDOIResolver_refactored.py
# ----------------------------------------
...
class SingleDOIResolver:
    ...
    def __init__(
        ...
        resolved_config = self.config_resolver.resolve_all_configuration(
            ...
            email_semantic_scholar, email_arxiv, sources, project
        )
        ...
        self.rate_limit_handler = RateLimitHandler(state_file=self.rate_limit_config["state_file"])
        ...
        logger.debug(f"SingleDOIResolver initialized with sources: {self.sources}")
        logger.debug(f"Email configuration: {list(self.email_config.keys())}")
        logger.debug(f"Rate limit state file: {self.rate_limit_config['state_file']}")
        logger.info("SingleDOIResolver initialized with focused single-responsibility components")
    ...
    def _check_scholar_library(self, title: str, year: Optional[int] = None) -> Optional[str]:
    ...
    def _save_to_scholar_library(
        self, title: str, doi: str, year: Optional[int] = None,
        authors: Optional[List[str]] = None, source: str = None,
        metadata: Optional[Dict] = None, bibtex_source: Optional[str] = None
    ):
    ...
    def _save_unresolved_entry(
        ...
        authors: Optional[List[str]] = None, bibtex_source: Optional[str] = None
    ):
        """Save unresolved entry to master Scholar library (delegated to cache manager)."""
        return self.cache_manager.save_unresolved_entry(title, year, authors, bibtex_source)
    ...
    def copy_bibtex_to_library(self, bibtex_path: str, project_name: Optional[str] = None) -> str:
        ...
        return self.cache_manager.copy_bibtex_to_library(bibtex_path, project_name)
    ...
    def _create_project_symlink(self, paper_id: str, readable_name: str):
        ...
        return self.cache_manager._create_project_symlink(paper_id, readable_name)

    def _ensure_project_symlink(
        self, title: str, year: Optional[int] = None, authors: Optional[List[str]] = None
    ):
    ...
    async def resolve_async(
        ...
        if not skip_cache:
            ...
            if cached_doi:
                ...
                return {"doi": cached_doi, "source": "scholar_library_cache", "title": title}
    ...
    def resolve(
        ...
        try:
            ...
            if loop.is_running():
                ...
                logger.warning("Cannot run synchronous resolve in async context. Use resolve_async() instead.")
                return None
            else:
    ...
    def get_abstract(
        ...
        try:
            ...
            result = asyncio.run(
                self.orchestrator.resolve_async(title=title, year=year, authors=authors, enable_enrichment=True)
            )
    ...
    def extract_dois_from_text(self, text: str) -> List[str]:
        ...
        try:
            url_extractor = self.source_manager.get_source("url_extractor")
            if url_extractor and hasattr(url_extractor, 'extract_dois_from_text'):
                return url_extractor.extract_dois_from_text(text)
            return []
        except Exception as e:
    ...
    def validate_doi(self, doi: str) -> bool:
        ...
        try:
            url_extractor = self.source_manager.get_source("url_extractor")
            if url_extractor and hasattr(url_extractor, 'validate_doi'):
                return url_extractor.validate_doi(doi)
        ...
        try:
            ...
            return {
                ...
                "configuration": self.config_resolver.get_configuration_summary(
                    self.email_config, self.sources, self.project, self.api_keys
                )
            }
        except Exception as e:
    ...
    def validate_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            source_validation = self.source_manager.validate_source_configuration()
            
            # Combine validations
            combined_validation = {
                "valid": config_validation["valid"] and source_validation["valid"],
                "warnings": config_validation["warnings"] + source_validation["warnings"],
                "errors": config_validation["errors"] + source_validation["errors"],
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        resolver = SingleDOIResolver(
            ...
            sources=["url_extractor", "crossref"],
