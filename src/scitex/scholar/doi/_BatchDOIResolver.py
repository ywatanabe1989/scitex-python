#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-08-02 18:34:31 (ywatanabe)"
...
class BatchDOIResolver:
    ...
    def __init__(
        ...
        self.max_workers = max_workers or self.config.feature.get(
            "max_workers", 4
        )
        ...
        if progress_file:
            ...
            self.progress_file = (
                workspace_dir / f"doi_resolution_{timestamp}.progress.json"
            )

        # Set up cache directory using path manager
        if cache_dir:
            self.cache_dir = Path(cache_dir)
        else:
            self.cache_dir = self.config.path_manager.cache_dir / "doi"
    ...
    def _find_similar_papers(
        ...
        for i, paper1 in enumerate(papers):
            ...
            if len(group) > 1:
                ...
                logger.info(
                    f"Found {len(group)} similar papers for: {paper1.get('title', '')[:50]}..."
                )
    ...
    def _adaptive_rate_limit(self) -> float:
        ...
        if self.progress_data["statistics"]["rate_limited"] > 5:
            rate_info["adaptive_delay"] = min(
                2.0, rate_info["adaptive_delay"] * 1.5
            )
        ...
        # Decrease delay if we're successful
            ...
            if recent_success_rate > 0.8:
                rate_info["adaptive_delay"] = max(
                    0.05, rate_info["adaptive_delay"] * 0.9
                )
        ...
        try:
            ...
            if doi:
                ...
                with open(cache_file, "w") as f:
                    json.dump(
                        {"doi": doi, "timestamp": datetime.now().isoformat()},
                        f,
                    )
                ...
                for source in sources[:3]:
                    self._source_success_rates[journal][source] = (
                        self._source_success_rates[journal].get(source, 0)
                        * 0.9
                        + 0.1
                    )
    ...
    def resolve_batch(
        ...
        progress = ProgressDisplay(
            total=len(papers_to_process), description="Resolving DOIs"
        )

        # Process concurrently with asyncio
        logger.info(
            f"Processing {len(papers_to_process)} papers with {self.max_workers} workers..."
        )

        async def process_all():
            tasks = []
            for i, paper in enumerate(papers_to_process):
                task = self._resolve_single_async(
                    paper, i, len(papers_to_process)
                )
                tasks.append(task)
    ...
    def _show_final_summary(self):
        ...
        logger.info(
            f"Successfully resolved: {stats['resolved']} ({stats['resolved']/stats['total']*100:.1f}%)"
        )
        ...
        if self.progress_data["processing_times"]:
            avg_time = sum(self.progress_data["processing_times"]) / len(
                self.progress_data["processing_times"]
            )
