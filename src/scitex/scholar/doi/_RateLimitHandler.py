#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-08-04 08:00:00 (ywatanabe)"
# File: /home/ywatanabe/proj/SciTeX-Code/src/scitex/scholar/doi/_RateLimitHandler.py
# ----------------------------------------
from __future__ import annotations
import os
__FILE__ = (
    "./src/scitex/scholar/doi/_RateLimitHandler.py"
)
...
class RateLimitHandler:
    ...
    RATE_LIMIT_PATTERNS = {
        "crossref": {
            "http_codes": [429, 503],
            "error_messages": ["rate limit", "too many requests", "quota exceeded"],
    ...
    def _load_state(self):
        ...
        try:
            ...
            logger.info(f"Loaded rate limit states for {len(self.source_states)} sources")
    ...
    def detect_rate_limit(self, 
                         ...
                         exception: Optional[Exception] = None) -> Optional[RateLimitInfo]:
        ...
        if exception is not None:
            ...
            if any(keyword in error_str for keyword in ['timeout', 'connection', 'read timeout']):
    ...
    def _calculate_backoff_time(self, source: str, base_multiplier: float = 1.0) -> float:
    ...
    def record_rate_limit(self, rate_limit_info: RateLimitInfo):
        ...
        state.rate_limited_count += 1  # Track rate limiting incidents for adaptive logic
        ...
        active_rate_limits = sum(1 for s in self.source_states.values() if s.is_rate_limited)
        if active_rate_limits >= 2:
            self.global_backoff = max(self.global_backoff, rate_limit_info.wait_time * 0.5)
    ...
    def should_use_global_backoff(self) -> bool:
        ...
        rate_limited_count = sum(1 for s in self.source_states.values() if s.is_rate_limited)
    ...
    async def wait_with_countdown_async(self, wait_time: float, source: str = "API"):
        ...
        if wait_time > 10:
            ...
            while True:
                ...
                print(f"\rWaiting for {source}: {time_str} remaining...", end='', flush=True)
    ...
    def get_statistics(self) -> Dict[str, any]:
        ...
        stats = {
            ...
            'rate_limited_sources': sum(1 for s in self.source_states.values() if s.is_rate_limited),
    ...
    def get_adaptive_delay(self, source: Optional[str] = None) -> float:
        ...
        if source:
            ...
            if state.rate_limited_count > 5:
                ...
                logger.debug(f"Increased adaptive delay for {source}: {state.adaptive_delay:.3f}s")
            ...
            # Decrease delay if source is performing well
                ...
                if recent_success_rate > 0.8:
                    ...
                    logger.debug(f"Decreased adaptive delay for {source}: {state.adaptive_delay:.3f}s")
            
            return state.adaptive_delay
        else:
            # Global adaptive delay logic
            recent_success_rate = self.calculate_recent_success_rate()
            total_rate_limited = sum(state.rate_limited_count for state in self.source_states.values())
            ...
            if total_rate_limited > 5:
                ...
                logger.debug(f"Increased global adaptive delay: {self.global_adaptive_delay:.3f}s")
            ...
            # Decrease global delay if success rate is high
                ...
                logger.debug(f"Decreased global adaptive delay: {self.global_adaptive_delay:.3f}s")
...
if __name__ == "__main__":
    ...
    async def test_rate_limit_handler_async():
        ...
        if rate_limit:
            print(f"   ✅ Detected rate limit: {rate_limit.limit_type.value} (wait {rate_limit.wait_time}s)")
        ...
        available_sources = handler.get_available_sources(["crossref", "pubmed", "semantic_scholar"])
        print(f"   Available sources: {available_sources}")
        
        rate_limited_sources = [s for s in ["crossref", "pubmed", "semantic_scholar"] 
        ...
        for source, details in stats['sources'].items():
            print(f"   {source}: success_rate={details['success_rate']:.2f}, "
