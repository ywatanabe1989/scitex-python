#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-07-31 21:23:53 (ywatanabe)"
# File: /home/ywatanabe/proj/scitex_repo/src/scitex/scholar/browser/local/_ZenRowsBrowserManager.py
# ----------------------------------------
from __future__ import annotations
import os
__FILE__ = (
    "./src/scitex/scholar/browser/local/_ZenRowsBrowserManager.py"
)
    ...
    def _build_proxy_config(self) -> Dict[str, Any]:
        ...
        if self._proxy_username and self._proxy_password:
            ...
            if self._proxy_country:
                ...
                username = f"{self._proxy_username}-country-{self._proxy_country}"
                logger.info(
                    f"Using ZenRows proxy with country routing: {self._proxy_country.upper()}"
                )
            
            logger.info(
                f"Using ZenRows proxy credentials: {username}"
            )
            return {
