#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Command-line tool for validating downloaded PDFs."""
...
def validate_downloaded_pdfs(
    ...
    for pdf_path in pdf_files:
        ...
        file_result = {
            ...
            'overall_valid': content_result['valid'] and quality_result.get('quality_score', 0) > 0.5
        }
...
def print_validation_summary(results: Dict[str, Dict]):
    ...
    print(f"Suspicious:      {summary['suspicious']} ({summary['suspicious']/summary['total']*100:.1f}%)")
    ...
    if summary['invalid'] > 0:
        ...
        for filename, result in results['files'].items():
            if not result['overall_valid']:
                ...
                print(f"  - {filename}: {reason} (confidence: {confidence:.1%})")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate downloaded PDFs to ensure they contain main paper content",
...
Examples:
  ...
  python -m scitex.scholar.validate_pdfs --pdf-dir ./pdfs --move-invalid
    ...
    parser.add_argument(
        "--bibtex", "-b",
        type=str,
        help="BibTeX file for title matching"
    )
    ...
    try:
        results = validate_downloaded_pdfs(
            pdf_dir=pdf_dir,
