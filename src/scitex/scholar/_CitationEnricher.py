#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-07-23 15:52:26 (ywatanabe)"
# File: /home/ywatanabe/proj/scitex_repo/src/scitex/scholar/_CitationEnricher.py
# ----------------------------------------
import os
__FILE__ = (
    "./src/scitex/scholar/_CitationEnricher.py"
)
...
class CitationEnricher:
    ...
    async def _enrich_citations_async(
        ...
        for paper in papers:
            ...
            if paper.doi:
                citation_count, source = self._get_citation_count_crossref(
                    paper
                )
                if citation_count is not None:
                    ...
                    enriched_count += 1
                    logger.debug(
                        f"Added citation count {citation_count} (from {source}) to: {paper.title[:50]}..."
                    )
                    continue
            ...
            if citation_count is not None:
                ...
                enriched_count += 1
                logger.debug(
                    f"Added citation count {citation_count} (from Semantic Scholar) to: {paper.title[:50]}..."
                )

        logger.info(
            f"Enriched {enriched_count}/{len(papers)} papers with citation counts"
        )
        return papers

    def _get_citation_count_crossref(
        self, paper: Paper
        ...
        try:
            ...
            for result in results:
                ...
                if (
                    paper.doi
                    and result.doi
                    and paper.doi.lower() == result.doi.lower()
                ):
                    return result.citation_count

        except Exception as e:
            logger.debug(
                f"Failed to get citation count for '{paper.title[:50]}...': {e}"
            )
