#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-08-07 22:59:46 (ywatanabe)"
...
async def _download_file(page: Page, save_path: str, url: str, selector: str):
    ...
    try:
        # Create tasks for both potential outcomes: a new page or a download.
        ...
        # Use force=True to click through any overlays (like cookie banners)
        ...
        # Cancel the event that didn't happen to prevent hanging
        for task in pending:
            task.cancel()
        ...
        elif completed_task == download_task:
        ...
        # Fallback to direct download if clicking fails
        try:
...
async def download_article_and_supplements(
    ...
    # 1. EXPOSE and log the main PDF URL first
    ...
    # 2. EXPOSE and log Supplementary File URLs
    ...
    for i, link in enumerate(supplementary_links):
        ...
        logger.success(
            f"Found Supplementary File #{i+1} ({link['text']}): {supp_url}"
        )
