...
class ShibbolethDownloadStrategy(BaseDownloadStrategy):
    ...
    async def can_download(self, url: str, paper: Optional[Any] = None) -> bool:
        ...
        Returns:
            True if Shibboleth is configured and authenticated
        """
    ...
    async def download(
        ...
        try:
            ...
            if not await self.authenticator.is_authenticated():
                ...
                await self.authenticator.authenticate(resource_url=url)
            
            logger.info(f"Downloading via Shibboleth: {url}")
            
            # Create authenticated browser
            browser, context = await self.authenticator.create_authenticated_browser()
            ...
            try:
                ...
                if await self._needs_institutional_login(page):
                    # Click institutional login
                    login_link = await self._find_institutional_login(page)
                    if login_link:
    ...
    async def _needs_institutional_login(self, page: Page) -> bool:
    ...
    async def _find_pdf_link(self, page: Page) -> Optional[str]:
        """
    ...
    async def get_authenticated_session(self) -> Dict[str, Any]:
        ...
        return {
            "cookies": await self.authenticator.get_auth_cookies(),
