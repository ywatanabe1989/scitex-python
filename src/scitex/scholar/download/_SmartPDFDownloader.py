...
class DownloadAgent:
    ...
    async def can_handle(self, paper: Paper, url: str) -> bool:
...
class AuthenticatedDownloadAgent(DownloadAgent):
    ...
    async def download(self, paper: Paper, url: str, output_path: Path) -> bool:
        ...
        try:
            ...
            browser = await self.auth_manager.get_authenticated_browser()
            
            if browser:
                page = await browser.new_page()
                
                # Use browser agent with authenticated session
                agent = BrowserDownloadAgent(None)
                agent.browser_manager = type('obj', (object,), {'get_browser': lambda: browser})()
...
class SmartPDFDownloader:
    ...
    def __init__(self, config: Optional[ScholarConfig] = None):
        ...
        # Download tracking
        self.download_dir = Path.home() / "Downloads" / "scitex_pdfs"
    ...
    def _get_output_path(self, paper: Paper) -> Path:
        """Generate output path for PDF."""
        # Format: FIRSTAUTHOR-YEAR-JOURNAL.pdf
        ...
        # Clean journal name
        journal = paper.journal or paper.venue or "Unknown"
        journal = journal.split()[0] if journal else "Unknown"
        journal = ''.join(c for c in journal if c.isalnum())[:20]
    ...
    async def download_single(self, paper: Paper) -> Tuple[bool, Optional[Path]]:
        ...
        if paper_id in self.progress.get('downloaded', {}):
            pdf_path = Path(self.progress['downloaded'][paper_id]['path'])
            if pdf_path.exists():
        ...
        # Try each URL with different agents
        ...
        for url in urls:
            ...
            for agent in agents:
                if await agent.can_handle(paper, url):
                    ...
                    if success and output_path.exists():
                        ...
                        if len(content) > 1000 and content.startswith(b'%PDF'):
                            # Track success
                            self.progress['downloaded'][paper_id] = {
