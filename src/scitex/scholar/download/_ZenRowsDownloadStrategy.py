...
class ZenRowsDownloadStrategy(BaseDownloadStrategy):
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    ...
                    if 'application/pdf' in content_type:
                        ...
                        pdf_url = await self._find_pdf_url(html_content, final_url)
                        
                        if pdf_url:
                            logger.info(f"Found PDF URL: {pdf_url}")
                            # Download the PDF
                            return await self._download_pdf_url(pdf_url, output_path)
                        else:
    ...
    async def _find_pdf_url(self, html_content: str, base_url: str) -> Optional[str]:
    ...
    async def _download_pdf_url(self, pdf_url: str, output_path: Path) -> Optional[Path]:
