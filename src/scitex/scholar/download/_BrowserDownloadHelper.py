...
<head>
    ...
    <script>
        function updateProgress() {{
            ...
            const downloaded = document.querySelectorAll('.paper.downloaded').length;
            document.getElementById('progress').textContent = `Downloaded: ${{downloaded}}/${{total}}`;
        }}
        
        function markDownloaded(index) {{
            const paper = document.getElementById(`paper-${{index}}`);
            paper.classList.toggle('downloaded');
            updateProgress();
            
            // Save state to localStorage
            const downloaded = [];
            document.querySelectorAll('.paper.downloaded').forEach(p => {{
                downloaded.push(p.id);
            }});
            localStorage.setItem('downloaded-{session_id}', JSON.stringify(downloaded));
        }}
        
        // Restore state on page load
        window.onload = function() {{
            const saved = localStorage.getItem('downloaded-{session_id}');
            if (saved) {{
                const downloaded = JSON.parse(saved);
                downloaded.forEach(id => {{
                    document.getElementById(id)?.classList.add('downloaded');
                }});
            }}
            updateProgress();
        }};
...
<body>
    ...
    <div class="stats">
        ...
        <p><strong>Instructions:</strong> Click on the links to open papers in your authenticated browser. 
        Check the box when downloaded. Your progress is automatically saved.</p>
    </div>
"""
