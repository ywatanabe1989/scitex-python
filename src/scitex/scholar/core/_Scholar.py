...
from scitex.scholar.metadata.doi._SingleDOIResolver import SingleDOIResolver
from scitex.scholar.metadata.enrichment._MetadataEnricher import (
    MetadataEnricher,
)

# SmartPDFDownloader removed - using PDFDownloader directly
from scitex.scholar.search._UnifiedSearcher import UnifiedSearcher
...
class Scholar:
    ...
    def search(
        ...
        **kwargs,
        ...
        if (
            self.config.resolve("enable_auto_enrich", None, True, bool)
            and papers
        ):
        ...
        if (
            self.config.resolve("enable_auto_download", None, False, bool)
            and papers
        ):
    ...
    def configure_openathens(
        ...
        self._pdf_downloader = PDFDownloader(
            download_dir=(
                Path(
                    self.config.resolve("pdf_dir", None, None, str)
                ).expanduser()
            ...
            use_lean_library=self.config.resolve(
                "use_lean_library", None, True, bool
            ),
            timeout=30,
            max_retries=3,
            max_concurrent=self.config.resolve(
                "max_parallel_requests", None, 3, int
            ),
            debug_mode=self.config.resolve("debug_mode", None, False, bool),
        )
    ...
    async def authenticate_async_openathens_async(
    ...
    def _initialize_pdf_downloader(self):
        ...
        if self.config.resolve("openathens_enabled", None, True, bool):
            openathens_config = {
                "email": self.config.resolve(
                    "openathens_email", None, None, str
                ),
                "debug_mode": self.config.resolve(
                    "debug_mode", None, False, bool
                ),
            }
        ...
        self._pdf_downloader = PDFDownloader(
            ...
            use_translators=True,
            use_playwright=True,
            use_openathens=self.config.resolve(
                "openathens_enabled", None, True, bool
            ),
            openathens_config=openathens_config,
            use_lean_library=self.config.resolve(
                "use_lean_library", None, True, bool
    ...
    def _print_config_summary(self):
        ...
        semantic_key = self.config.resolve(
        ...
        if semantic_key:
            ...
            print(
                "  • Semantic Scholar: ✗ Not set (citations via CrossRef only)"
            )
        ...
        if pubmed_email:
            ...
            print("  • PubMed Email: ✗ Not set (required for PubMed)")

        # Features
        print("\n⚙️  Features:")

        # Auto-enrichment
        auto_enrich = self.config.resolve(
            "enable_auto_enrich", None, True, bool
        )
        print(
            f"  • Auto-enrichment: {'✓ Enabled' if auto_enrich else '✗ Disabled'}"
        )

        print("  • Impact factors: ✓ Using JCR package (2024 data)")

        # Auto-download
        auto_download = self.config.resolve(
            "enable_auto_download", None, False, bool
        )
        print(
        ...
        # OpenAthens status
        openathens_enabled = self.config.resolve(
            "openathens_enabled", None, True, bool
        )
        if openathens_enabled:
            ...
            if openathens_email:
                ...
                if len(parts) == 2:
                    masked_email = f"{mask_sensitive(parts[0], 2)}@{parts[1]}"
                    print(f"  • OpenAthens: ✓ Enabled ({masked_email})")
                else:
                    print("  • OpenAthens: ✓ Enabled")
            else:
                print("  • OpenAthens: ✓ Enabled (no email set)")
        else:
            print("  • OpenAthens: ✗ Disabled")

        # Settings
        print("\n📁 Settings:")
        print(f"  • Workspace: {self.workspace_dir}")

        default_limit = self.config.resolve(
            "default_search_limit", None, 20, int
        )
        print(f"  • Default search limit: {default_limit}")

        default_sources = self.config.resolve(
            "default_search_sources", None, [], list
        )
        if default_sources:
            print(f"  • Default sources: {', '.join(default_sources)}")
        else:
            print("  • Default sources: None configured")
    ...
    async def authenticate_async_ezproxy_async(
        self, force: bool = False
    ) -> bool:
        """
    ...
    async def authenticate_async_shibboleth_async(
        self, force: bool = False
    ) -> bool:
        """
