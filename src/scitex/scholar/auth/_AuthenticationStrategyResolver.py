#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-08-05 20:33:00 (ywatanabe)"
...
class AuthenticationStrategyResolver:
    ...
    def resolve_strategy(
        ...
        if not openathens_email:
            logger.warning("No email provided - defaulting to manual authentication")
        ...
        if institution_config:
            return self._resolve_known_institution_strategy(
                openathens_email, email_domain, institution_config, 
                target_url, prefer_openathens
            )
        else:
    ...
    def _resolve_known_institution_strategy(
        ...
        logger.info(f"Resolving strategy for known institution: {institution_name}")
        ...
        if prefer_openathens and institution_config.get('openathens_supported', False):
            ...
            fallback_methods = [
                ...
                AuthenticationMethod.MANUAL
            ] if institution_config.get('openathens_supported', False) else [
                AuthenticationMethod.MANUAL
            ]
    ...
    def _resolve_unknown_institution_strategy(
        ...
        logger.info(f"Resolving strategy for unknown institution: {email_domain}")
    ...
    def get_supported_institutions(self) -> List[str]:
        ...
        return [
            config['name'] for config in self.INSTITUTION_CONFIGS.values()
        ]
...
if __name__ == "__main__":
    def main():
        ...
        print(f"UniMelb Strategy: {strategy.method.value}, URL: {strategy.primary_url}, Confidence: {strategy.confidence}")
        
        # Test unknown institution
        strategy = resolver.resolve_strategy(
            openathens_email="test@unknown.edu"
        )
        print(f"Unknown Strategy: {strategy.method.value}, URL: {strategy.primary_url}, Confidence: {strategy.confidence}")
        
        # Test manual fallback
        strategy = resolver.resolve_strategy()
        print(f"No Email Strategy: {strategy.method.value}, URL: {strategy.primary_url}, Confidence: {strategy.confidence}")
