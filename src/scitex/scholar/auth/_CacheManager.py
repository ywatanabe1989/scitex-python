...
class CacheManager:
    ...
    def __init__(self, cache_name: str, config: ScholarConfig, email: Optional[str] = None):
    ...
    def _validate_cache_data(self, cache_data: Dict[str, Any]) -> bool:
        ...
        if self.email:
            ...
            if cached_email and cached_email.lower() != self.email.lower():
                logger.debug(f"Email mismatch: cached={cached_email}, current={self.email}")
                return False

        return True

    def _populate_session_manager(self, session_manager: SessionManager, cache_data: Dict[str, Any]) -> None:
