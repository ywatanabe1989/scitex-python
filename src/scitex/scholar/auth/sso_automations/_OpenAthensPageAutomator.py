#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-08-05 20:31:00 (ywatanabe)"
# File: /home/ywatanabe/proj/scitex_repo/src/scitex/scholar/auth/sso_automations/_OpenAthensPageAutomator.py
# ----------------------------------------
from __future__ import annotations
import os
__FILE__ = (
    "./src/scitex/scholar/auth/sso_automations/_OpenAthensPageAutomator.py"
)
...
class OpenAthensPageAutomator(BaseSSOAutomator):
    ...
    async def _enter_institutional_email_async(self, page) -> bool:
        ...
        try:
            self.logger.info(f"Entering institutional email: {self.openathens_email}")
        ...
        try:
            ...
            for selector in submit_selectors:
                try:
                    ...
                    if element:
                        success = await BrowserUtils.reliable_click_async(page, selector)
                        if success:
                            self.logger.info(f"Successfully clicked submit using: {selector}")
                            await page.wait_for_timeout(1000)
                            return True
                except TimeoutError:
                    continue

            self.logger.info("No explicit submit button found, assuming form submitted automatically")
            return True
        ...
        try:
            ...
            try:
                ...
                if len(institution_elements) == 1:
                    ...
                    self.logger.success(f"Selected institution: {institution_text.strip()}")
                    
                elif len(institution_elements) > 1:
                    # Multiple options - try to find best match based on email domain
                    selected = await self._select_best_institution_match(page, institution_elements)
                    if not selected:
                        # If no good match, select the first one
                        await institution_elements[0].click()
                        institution_text = await institution_elements[0].text_content()
                        self.logger.info(f"Selected first available institution: {institution_text.strip()}")
                else:
    ...
    async def _select_best_institution_match(self, page, institution_elements) -> bool:
        ...
        try:
            email_domain = self.openathens_email.split('@')[1].lower() if '@' in self.openathens_email else ""
            ...
            if email_domain in domain_keywords:
                ...
                for element in institution_elements:
                    ...
                    if any(keyword in text_lower for keyword in keywords):
                        await element.click()
                        self.logger.success(f"Selected matched institution: {text.strip()}")
                        return True
            ...
            if 'university' in email_domain or 'edu' in email_domain:
                for element in institution_elements:
                    ...
                    if 'university' in text.lower():
                        await element.click()
                        self.logger.info(f"Selected university institution: {text.strip()}")
                        return True
        ...
        try:
            ...
            for i in range(30):  # Wait up to 30 seconds
                ...
                if not self.is_sso_page(current_url):
                    self.logger.success(f"Redirected to institution SSO: {current_url}")
                    return True
...
if __name__ == "__main__":
    ...
    def main():
        ...
        async def test_automator():
            automator = OpenAthensPageAutomator()
...
# python -m scitex.scholar.auth.sso_automations._OpenAthensPageAutomator
