...
try:
    ...
    logger.warning("impact_factor package not available. Install with: pip install impact_factor")


class ImpactFactorService:
    """Service for retrieving real impact factors using impact_factor package."""
    
    def __init__(self, year: Optional[int] = None, cache_dir: Optional[Path] = None):
        """
        ...
        if IMPACT_FACTOR_AVAILABLE:
            try:
                ...
                logger.error(f"Failed to initialize impact factor service: {e}")
    ...
    def get_journal_metrics(self, journal_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not results:
                logger.debug(f"No impact factor found for journal: {journal_name}")
                return {}
            ...
            logger.info(f"Found IF={metrics['impact_factor']} for {journal_name}")
            return metrics
            
        except Exception as e:
            logger.error(f"Error retrieving impact factor for {journal_name}: {e}")
            return {}
    ...
    def batch_get_metrics(self, journal_names: List[str]) -> Dict[str, Dict[str, Any]]:
        """
...
class EnhancedJournalMetrics:
    ...
    def __init__(self, custom_db_path: Optional[str] = None, use_impact_factor_package: bool = True):
        """
    ...
    def lookup_journal_metrics(self, journal_name: str) -> Dict[str, Any]:
        ...
        if self.if_service and self.if_service.is_available():
            ...
            if if_metrics:
                ...
                if 'impact_factor' in if_metrics and if_metrics['impact_factor'] > 0:
                    metrics['impact_factor'] = if_metrics['impact_factor']
                    metrics['impact_factor_year'] = if_metrics.get('year', 2024)
...
def install_impact_factor_package():
    ...
    try:
        subprocess.check_call([sys.executable, "-m", "pip", "install", "impact_factor"])
