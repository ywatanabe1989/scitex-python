#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-08-09 02:54:40 (ywatanabe)"
# File: /home/ywatanabe/proj/SciTeX-Code/src/scitex/scholar/metadata/doi/resolvers/_EnhancedSingleDOIResolver.py
# ----------------------------------------
from __future__ import annotations
import os
__FILE__ = (
    "./src/scitex/scholar/metadata/doi/resolvers/_EnhancedSingleDOIResolver.py"
)
...
1. URLDOISource - Extract DOIs from URL fields
...
from ..sources._URLDOISource import URLDOISource
...
class EnhancedSingleDOIResolver:
    ...
    def __init__(
        ...
        if self.enable_utilities:
            self.url_doi_source = URLDOISource()
            ...
            logger.debug(
                "Enhanced DOI resolver initialized with Phase 1 utilities"
            )
        else:
            self.url_doi_source = None
            self.pubmed_converter = None
            self.text_normalizer = None

            logger.debug(
                "Enhanced DOI resolver initialized without utilities (compatibility mode)"
            )
    ...
    def _try_utility_extraction(self, entry: Dict) -> Optional[Dict]:
        ...
        doi = self.url_doi_source.extract_from_bibtex_entry(entry)
        if doi:
    ...
    async def resolve_async(
        ...
        normalized_title, normalized_authors = (
            self._normalize_search_parameters(title, authors)
        )
    ...
    def get_enhancement_stats(self) -> Dict[str, int]:
        ...
        return {
            ...
            "url_doi_source_ready": self.url_doi_source is not None,
...
def main():
    ...
    test_entries = [
        ...
        {"title": "Test Paper with PMID", "pmid": "25821343", "year": "2015"},
        {
    ...
    for i, entry in enumerate(test_entries):
        ...
        if utility_result:
            print(
                f"   ✅ Utility extraction: {utility_result['doi']} (via {utility_result['source']})"
            )
        else:
        ...
        norm_title, norm_authors = (
            enhanced_resolver._normalize_search_parameters(title, authors)
        )
    ...
    compat_resolver = EnhancedSingleDOIResolver(
        email_crossref="test@example.com", enable_utilities=False
    )
    ...
    print(
        "2. Compatibility mode: EnhancedSingleDOIResolver(enable_utilities=False)"
    )
    print(
        "3. With entry: resolver.resolve_async(title, year, authors, entry=entry)"
    )
...
# python -m scitex.scholar.doi.utils.EnhancedDOIResolver
