#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-08-04 18:20:00 (ywatanabe)"
# File: ./src/scitex/scholar/doi/sources/_URLDOIExtractor.py
# ----------------------------------------
...
class URLDOIExtractor(BaseDOISource):
    ...
    def __init__(self):
        ...
        if self.api_key:
            logger.info("Using Semantic Scholar API key for enhanced CorpusId resolution")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if ieee_id.isdigit():
                    ...
                    potential_doi = f"10.1109/ACCESS.{ieee_id}"  # Example pattern
            ...
            try:
                ...
                if response.status_code == 429:  # Rate limited
                    if attempt < max_retries - 1:
                        ...
                        logger.info(f"Rate limited on Semantic Scholar CorpusId {semantic_id}, retrying in {delay:.1f}s...")
                        time.sleep(delay)
                        continue
                    else:
                        logger.warning(f"Rate limited on Semantic Scholar CorpusId {semantic_id}, giving up after {max_retries} attempts")
                        return None
                
                if response.status_code == 404:
                    logger.debug(f"Semantic Scholar ID {semantic_id} not found")
                    return None
                ...
                if response.status_code == 200:
                    ...
                    if doi:
                        logger.info(f"Successfully resolved Semantic Scholar ID {semantic_id} → DOI: {doi}")
                        return self._clean_doi(doi)
                    else:
                        # Check for ArXiv as fallback
                        arxiv_id = external_ids.get('ArXiv')
                        if arxiv_id:
                            logger.info(f"Semantic Scholar ID {semantic_id} has ArXiv ID: {arxiv_id} (no DOI)")
                        else:
                            logger.debug(f"Semantic Scholar ID {semantic_id} found but no DOI or ArXiv available")
                        return None
                ...
                logger.debug(f"Semantic Scholar HTTP error for {semantic_id}: {e}")
                return None
            except Exception as e:
                logger.debug(f"Semantic Scholar lookup failed for {semantic_id}: {e}")
                return None
    ...
    def __str__(self) -> str:
        ...
        total_patterns = len(self.ieee_patterns + self.pubmed_patterns + self.semantic_patterns)
        return f"URLDOIExtractor(enhanced_patterns={total_patterns}, uses_utils=True)"


# Export
__all__ = ["URLDOIExtractor"]
