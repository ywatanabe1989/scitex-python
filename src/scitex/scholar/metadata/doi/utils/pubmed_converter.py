#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-08-04 08:05:00 (ywatanabe)"
# File: /home/ywatanabe/proj/scitex_repo/src/scitex/scholar/doi/utils/pubmed_converter.py
# ----------------------------------------
from __future__ import annotations
import os
__FILE__ = (
    "./src/scitex/scholar/doi/utils/pubmed_converter.py"
)
...
class PubMedConverter:
    ...
    def pmid_to_doi(self, pmid: Union[str, int]) -> Optional[str]:
        """
    ...
    async def pmid_to_doi_async(self, pmid: Union[str, int]) -> Optional[str]:
        """
    ...
    def extract_from_bibtex_entry(self, entry: Dict) -> Optional[str]:
        """
        ...
        for field in pmid_fields:
            ...
            if value:
                ...
                if pmid:
                    ...
                    if doi:
                        logger.info(f"Converted PMID to DOI for '{entry.get('title', 'Unknown')}': {pmid} → {doi}")
                        return doi
                        
        return None
        
    def extract_from_bibtex_entries(self, entries: List[Dict]) -> Dict[str, str]:
        """
        ...
        for i, entry in enumerate(entries):
            extracted_doi = self.extract_from_bibtex_entry(entry)
            if extracted_doi:
                results[str(i)] = extracted_doi
                
        if results:
            logger.info(f"PubMedConverter: Converted PMIDs to DOIs for {len(results)} entries")
            
        return results
        
    async def extract_from_bibtex_entries_async(self, entries: List[Dict]) -> Dict[str, str]:
        """
        ...
        for i, entry in enumerate(entries):
            ...
            for field in pmid_fields:
                ...
                if value:
                    pmid = self._extract_pmid_from_string(str(value))
                    if pmid:
                        tasks.append(self.pmid_to_doi_async(pmid))
        ...
        if converted_dois:
            logger.info(f"PubMedConverter: Converted PMIDs to DOIs for {len(converted_dois)} entries")
...
def main():
    ...
    for pmid in test_pmids:
        result = converter.pmid_to_doi(pmid)
    ...
    for i, entry in enumerate(test_entries):
        result = converter.extract_from_bibtex_entry(entry)
        status = "✅" if result else "❌"
        print(f"   {status} Entry {i}: '{entry['title']}' → {result}")
        
    # Test batch conversion
    print("\n3. Testing batch conversion:")
    batch_results = converter.extract_from_bibtex_entries(test_entries)
    print(f"   📊 Converted PMIDs to DOIs for {len(batch_results)} out of {len(test_entries)} entries")
    ...
    print("=" * 60)
    print("\nUsage patterns:")
    print("1. Single PMID: converter.pmid_to_doi(pmid)")
    print("2. BibTeX entry: converter.extract_from_bibtex_entry(entry)")
    print("3. Batch entries: converter.extract_from_bibtex_entries(entries)")
    print("4. Async batch: converter.extract_from_bibtex_entries_async(entries)")
...
# python -m scitex.scholar.doi.utils.pubmed_converter
