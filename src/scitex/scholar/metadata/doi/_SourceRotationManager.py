...
# File: /home/ywatanabe/proj/SciTeX-Code/src/scitex/scholar/doi/_SourceRotationManager.py
# ----------------------------------------
from __future__ import annotations
import os
__FILE__ = (
    "./src/scitex/scholar/doi/_SourceRotationManager.py"
)
...
class SourceRotationManager:
    ...
    def __init__(self, rate_limit_handler: RateLimitHandler):
        ...
        self.recent_attempts = deque(maxlen=100)  # Track recent attempts for adaptive behavior
    ...
    def record_attempt(self, source: str, paper_info: Dict, success: bool, response_time: float = 0.0):
        ...
        recent_attempts_for_source = [
            attempt for attempt in list(self.recent_attempts)[-20:]
            if attempt['source'] == source
        ]
        if recent_attempts_for_source:
            recent_successes = sum(1 for a in recent_attempts_for_source if a['success'])
            perf.recent_success_rate = recent_successes / len(recent_attempts_for_source)
    ...
    def _classify_paper(self, paper_info: Dict) -> str:
        ...
        # Check for preprints
        preprint_indicators = ['arxiv', 'biorxiv', 'medrxiv', 'preprint', 'submitted']
        if any(indicator in journal or indicator in title for indicator in preprint_indicators):
            return 'preprints'
        
        # Check for biomedical/medical
        biomedical_terms = [
            'medicine', 'medical', 'biology', 'biomedical', 'clinical', 'health',
            'disease', 'therapy', 'treatment', 'patient', 'cell', 'molecular',
            'genetics', 'pharmaceutical', 'drug'
        ]
        ...
        cs_terms = [
            'computer', 'computing', 'algorithm', 'machine learning', 'artificial intelligence',
            'neural network', 'deep learning', 'data science', 'software', 'programming'
        ]
        if any(term in title or term in journal for term in cs_terms):
            return 'computer_science'
        
        # Check for physics/mathematics
        physics_terms = ['physics', 'quantum', 'relativity', 'particle', 'cosmology']
        math_terms = ['mathematics', 'mathematical', 'theorem', 'proof', 'algebra']
        ...
        conference_indicators = [
            'proceedings', 'conference', 'workshop', 'symposium', 'ieee', 'acm'
        ]
    ...
    def get_optimal_source_order(self, 
                                paper_info: Dict,
    ...
    def _calculate_source_score(self, source: str, paper_type: str, paper_info: Dict) -> float:
        ...
        if perf.total_attempts > 5:  # Only consider if we have enough data
            ...
            combined_rate = (overall_rate * 0.3) + (recent_rate * 0.7)  # Favor recent performance
        ...
        if rate_limit_state.is_rate_limited:
            ...
            rate_limit_factor = max(0.5, 1.0 - (rate_limit_state.consecutive_failures * 0.1))
    ...
    def get_fallback_sources(self, 
                           original_sources: List[str], 
                           all_sources: List[str]) -> List[str]:
        ...
        available_fallbacks = self.rate_limit_handler.get_available_sources(untried_sources)
    ...
    def get_statistics(self) -> Dict:
        ...
        stats = {
            ...
            'total_attempts': sum(p.total_attempts for p in self.source_performance.values()),
        ...
        total_successes = sum(p.successful_resolutions for p in self.source_performance.values())
        if total_attempts > 0:
...
if __name__ == "__main__":
    ...
    async def test_source_rotation_manager():
        ...
        test_papers = [
            {"title": "Deep Learning for Computer Vision", "journal": "IEEE Transactions"},
            {"title": "Clinical Trial of New Cancer Treatment", "journal": "Nature Medicine"},
            {"title": "Quantum Computing Algorithm", "journal": "arXiv preprint"},
            {"title": "Mathematical Proof of Fermat's Theorem", "journal": "Journal of Mathematics"}
        ]
        ...
        cs_paper = {"title": "Machine Learning in Healthcare", "journal": "AI Conference"}
        ...
        all_sources = ["crossref", "semantic_scholar", "pubmed", "openalex", "arxiv"]
        ...
        for source, details in stats['source_details'].items():
            print(f"   {source}: {details['successes']}/{details['attempts']} "
