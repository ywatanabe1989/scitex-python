...
class ResolutionOrchestrator:
    ...
    def __init__(
        ...
        logger.debug(
            f"ResolutionOrchestrator initialized for project '{project}'"
        )
    ...
    def enrichment_pipeline(self):
        ...
        if self._enrichment_pipeline is None:
            try:
                from ....metadata.enrichment._EnricherPipeline import (
                    EnricherPipeline,
                )

                self._enrichment_pipeline = EnricherPipeline(
                    email_crossref=self.enrichment_config.get(
                        "email_crossref", "research@example.com"
                    ),
                    email_pubmed=self.enrichment_config.get(
                        "email_pubmed", "research@example.com"
                    ),
                    email_openalex=self.enrichment_config.get(
                        "email_openalex", "research@example.com"
                    ),
                    email_semantic_scholar=self.enrichment_config.get(
                        "email_semantic_scholar", "research@example.com"
                    ),
                    semantic_scholar_api_key=self.enrichment_config.get(
        ...
        **kwargs,
        ...
        try:
            ...
            if existing_doi:
                ...
                return {"doi": existing_doi, "source": "scholar_library"}
            ...
            if enable_enrichment:
                logger.info(
                    "Enrichment temporarily disabled to prevent nested metadata structure - using source metadata only"
                )
            ...
            logger.error(
                f"Resolution orchestration failed for '{title[:50]}...': {e}"
            )
    ...
    def get_workflow_statistics(self) -> Dict[str, Any]:
        ...
        workflow_stats = {
            ...
            "enrichment_rate": (
                self.stats["enrichments"]
                / self.stats["source_resolutions"]
                * 100
                if self.stats["source_resolutions"] > 0
                else 0
            ),
...
if __name__ == "__main__":
    ...
    async def test_resolution_orchestrator():
        ...
        class MockConfig:
            class PathManager:
                ...
                def get_paper_storage_paths(self, paper_info, collection_name):
                    ...
                    storage_path = (
                        self.get_scholar_library_path()
                        / collection_name
                        / unique_id
                    )
                    readable_path = Path(
                        f"AUTHOR-{paper_info.get('year', 'UNKNOWN')}-JOURNAL"
                    )
                    return {
                        "unique_id": unique_id,
        ...
        print(
            f"   Source resolution rate: {stats['source_resolution_rate']:.1f}%"
        )
        ...
        print(
            f"   Throughput: {stats['throughput_per_minute']:.1f} papers/min"
        )
