...
class OptimizedInspector:
    ...
    def _print_table_info(self, result: Dict):
        ...
        if result["sample_data"]:
            ...
            print(f"\nFirst row (all {len(data_cols)} columns):")
            first_row = result["sample_data"][0]
            
            # Calculate max key length for alignment
            max_key_len = max(len(str(k)) for k in data_cols) if data_cols else 0
            max_key_len = min(max_key_len, 40)  # Cap at 40 chars for readability
            ...
            for key in data_cols:
                ...
                for col in result["columns"]:
                    if col["name"] == key:
                        ...
                        if constraints:
                            col_info = f" ({col_type}, {', '.join(constraints)})"
                        else:
                            col_info = f" ({col_type})"
                        break
                ...
                print(f"  {display_key:<{max_key_len}}: {display_value}{col_info}")
            ...
            if len(result["sample_data"]) > 1:
                ...
                print(f"\nAdditional samples (rows 2-{max_row} of {total_samples}, first 5 columns):")
