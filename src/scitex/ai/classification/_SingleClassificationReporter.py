#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-09-22 02:34:25 (ywatanabe)"
# File: /ssh:sp:/home/ywatanabe/proj/scitex_repo/src/scitex/ml/classification/_SingleClassificationReporter.py
# ----------------------------------------
from __future__ import annotations
import os
__FILE__ = (
    "./src/scitex/ml/classification/_SingleClassificationReporter.py"
)
    ...
    **kwargs
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        if confusion_matrices:
            ...
            summary["overall_confusion_matrix_normalized"] = self._round_numeric(overall_cm_normalized.tolist())
    ...
    def create_cv_summary_curves(self, summary: Dict[str, Any]) -> None:
        ...
        self._save_cv_summary_curve_data(all_y_true, all_y_proba, roc_mean, roc_std, pr_mean, pr_std, n_folds)
        ...
        pr_filename = f"pr_curve_cv_summary_ap_{pr_mean:.3f}_{pr_std:.3f}_n{n_folds}.jpg"
        ...
        logger.info(f"Created CV summary ROC curve: AUC = {roc_mean:.3f} ± {roc_std:.3f} (n={n_folds})")
        logger.info(f"Created CV summary PR curve: AP = {pr_mean:.3f} ± {pr_std:.3f} (n={n_folds})")
    ...
    def save_cv_summary_confusion_matrix(self, summary: Dict[str, Any]) -> None:
        ...
        if "metrics_summary" in summary:
            if "balanced_accuracy" in summary["metrics_summary"]:
                balanced_acc_stats = summary["metrics_summary"]["balanced_accuracy"]
                ...
                try:
                    ...
                    for _ in range(2):
                        result = subprocess.run(
                            ['pdflatex', '-interaction=nonstopmode', 'classification_report.tex'],
                            capture_output=True,
                            text=True,
                            timeout=30
                        )
    ...
    def save_summary(
        ...
        try:
            ...
            possible_paths = [
                self.output_dir.parent / "CONFIGS" / "CONFIG.yaml",  # ../CONFIGS/CONFIG.yaml
                self.output_dir.parent.parent / "CONFIGS" / "CONFIG.yaml",  # ../../CONFIGS/CONFIG.yaml
    ...
    def _save_cv_summary_classification_report(self, summary: Dict[str, Any]) -> None:
        ...
        for report in all_reports:
            all_classes.update([k for k in report.keys() 
                              if k not in ['accuracy', 'macro avg', 'weighted avg']])
        
        # Process each class
        for cls in sorted(all_classes):
            cls_metrics = {'precision': [], 'recall': [], 'f1-score': [], 'support': []}
            ...
            for metric, values in cls_metrics.items():
                if values:
                    if metric == 'support':
                        ...
                        summary_report[cls][metric] = f"{mean_val:.3f} ± {std_val:.3f} (n={n_folds})"
            ...
            if any(avg_metrics.values()):
                ...
                for metric, values in avg_metrics.items():
                    if values:
                        mean_val = np.mean(values)
                        std_val = np.std(values)
                        summary_report[avg_type][metric] = f"{mean_val:.3f} ± {std_val:.3f} (n={n_folds})"
    ...
    def __repr__(self) -> str:
        ...
        return (
            ...
            f"name='{self.name}', folds={fold_count}, "
