#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-10-01 16:00:00 (ywatanabe)"
# File: /home/ywatanabe/proj/scitex_repo/src/scitex/stats/correct/_correct_sidak.py
# ----------------------------------------
from __future__ import annotations
import os
__FILE__ = (
    "./src/scitex/stats/correct/_correct_sidak.py"
)
...
def main():
    ...
    for i in range(5):
        ...
        results.append(r)
        logger.info(f"Test {i+1}: p = {r['pvalue']:.4f}")

    corrected = correct_sidak(results, alpha=0.05)

    logger.info(f"\nOriginal alpha: 0.05")
    logger.info(f"Šidák-adjusted alpha: {corrected[0]['alpha_adjusted']:.4f}")
    logger.info(f"\nAdjusted results:")
    for i, r in enumerate(corrected):
        logger.info(f"Test {i+1}: p_adj = {r['pvalue_adjusted']:.4f}, "
    ...
    logger.info(f"Difference: {corrected[0]['alpha_adjusted'] - bonf[0]['alpha_adjusted']:.6f}")
    ...
    corrected_20 = correct_sidak(results_20, alpha=0.05)
    bonf_20 = correct_bonferroni(results_20, alpha=0.05)
    ...
    logger.info(f"  Power gain: {(corrected_20[0]['alpha_adjusted'] / bonf_20[0]['alpha_adjusted'] - 1) * 100:.2f}%")

    # Example 4: Comparison with Holm
    logger.info("\n[Example 4] Šidák vs Holm (sequential Bonferroni)")
    logger.info("-" * 70)

    holm = correct_holm(results, alpha=0.05)

    n_rejected_holm = sum(r['rejected'] for r in holm['results'])
    ...
    df_corrected = correct_sidak(df_input, alpha=0.05)
    ...
    logger.info(df_corrected[['var_x', 'var_y', 'pvalue', 'pvalue_adjusted',
                              'rejected']].head(3).to_string())

    # Example 6: Single test
    logger.info("\n[Example 6] Single test (returns dict)")
    logger.info("-" * 70)

    single = correct_sidak(results[0], alpha=0.05)
    ...
    edge_corrected = correct_sidak(edge_results, alpha=0.05)

    for i, (orig, corr) in enumerate(zip(edge_results, edge_corrected)):
        logger.info(f"Test {i+1}: p = {orig['pvalue']:.4f} -> "
    ...
    for alpha_val in [0.01, 0.05, 0.10]:
        corr = correct_sidak(results, alpha=alpha_val)
        n_rej = sum(r['rejected'] for r in corr)
        logger.info(f"Alpha = {alpha_val:.2f}: alpha_adj = {corr[0]['alpha_adjusted']:.4f}, "
    ...
    # Save as Excel
    convert_results(df_corrected, return_as='excel',
                   path='./sidak_corrected.xlsx')
    logger.info("Saved to: ./sidak_corrected.xlsx")

    # Save as CSV
    convert_results(df_corrected, return_as='csv',
                   path='./sidak_corrected.csv')
    logger.info("Saved to: ./sidak_corrected.csv")
